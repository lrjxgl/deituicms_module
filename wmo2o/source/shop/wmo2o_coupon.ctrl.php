<?php
	...
	class wmo2o_couponControl extends skymvc{
		...
		public function onDefault(){
			$where=" shopid=".SHOPID;
			$url="/moduleshop.php?m=wmo2o_coupon&a=default";
			$limit=20;
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_wmo2o_coupon")->selectRow(array("where"=>"id={$id}"));
			...
			$this->smarty->display("wmo2o_coupon/add.html");
		}
		
		public function onSave(){
			
			$id=get_post("id","i");
			$data=M("mod_wmo2o_coupon")->postData();
			$data['shopid']=SHOPID;
			$data["status"]=1;
			if($id){
				$row=M("mod_wmo2o_coupon")->selectRow(array("where"=>"id={$id}"));
				if($row['shopid']!=SHOPID ){
					$this->goAll("暂无权限");
				}
				M("mod_wmo2o_coupon")->update($data,"id='$id'");
			}else{
				 
				$data["dateline"]=time();
				M("mod_wmo2o_coupon")->insert($data);
			}
		...
		public function onSend(){
			...
			M("mod_wmo2o_coupon_user")->insert(array(
				"coupon_id"=>$id,
				"userid"=>$userid,
				"dateline"=>time()
			));
		...
		public function onUser(){
			...
			$limit=20;
			$where=" 1=1 ";
			$url="/moduleshop.php?m=wmo2o_coupon&a=user";
			$option=array(
				"where"=>$where,
				"start"=>$start,
				"limit"=>$limit
			...
			if($data){
				...
				$cids && $cos=M("mod_wmo2o_coupon")->getByIds($cids);
				foreach($data as $k=>$v){
		...
		public function onuserDelete(){
			$id=get_post('id',"i");
			M("mod_wmo2o_coupon_user")->delete("id={$id}");
			exit(json_encode(array("error"=>0,"message"=>"删除成功")));
		}

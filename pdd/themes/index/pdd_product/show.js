...
$(document).on("click",".ppBox-Show",function(){
	if($(this).attr("ispin")=="1"){
		ispin=true;
		$("#ppBox-price").addClass("none");
		$("#ppBox-pt-price").removeClass("none");
	}else{
		ispin=false;
		$("#ppBox-price").removeClass("none");
		$("#ppBox-pt-price").addClass("none");
	}
...
$(document).on("click", "#ks1 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=pdd_product_ks&a=sizeList&ajax=1&id=" + kid,
		dataType: "json",
...
$(document).on("click", "#ks2 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=pdd_product_ks&a=get&ajax=1&id=" + kid,
		dataType: "json",
		success: function(res) {
			that.addClass("kslist-active").siblings().removeClass("kslist-active");
			ksid = res.data.ksid;
	...
	$.ajax({
		...
		success: function(res) {
			skyToast(res.message);
			 ...
			 if(ispin){
				window.location="/module.php?m=pdd_order&a=confirm&shopid="+shopid+"&pin_orderid="+pin_orderid+"&ispin=1&cartid="+res.data.cartid 
			 }else{
				 window.location="/module.php?m=pdd_order&a=confirm&shopid="+shopid+"&pin_orderid="+pin_orderid+"&cartid="+res.data.cartid
			 }

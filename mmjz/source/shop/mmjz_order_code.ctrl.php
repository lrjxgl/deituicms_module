<?php
class mmjz_order_codeControl extends skymvc{
	
	public function __construct(){
		parent::__construct();
	}
	public function onDefault(){
		$this->smarty->display("mmjz_order_code/index.html");
	}
	
	public function onCheck(){
		$_GET["ajax"]=1;
		$ordercode=get("ordercode","h");
		$row=M("mod_mmjz_order_code")->selectRow("ordercode='".$ordercode."'");
		if(empty($row)){
			$this->goAll("验证码错误",1);
		}
	...
	public function onFinish(){
		...
		$order=MM("mmjz","mmjz_order")->selectRow("orderid=".$row["orderid"]);
		...
		M("mod_mmjz_order_code")->update(array(
			"isuse"=>1			
		),"orderid=".$row["orderid"]);
		$res=MM("mmjz","mmjz_order")->finish($row["orderid"]);

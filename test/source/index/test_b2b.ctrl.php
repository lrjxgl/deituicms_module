<?php
class test_b2bControl extends skymvc{
	public $md="b2b";
	...
	public function onNav(){
		$catids=M("mod_b2b_category")->selectCols(array(
			"where"=>" status=1 ",
			"fields"=>" catid"
		));
	...
	public function onSHop(){
		...
		foreach($arr as $a){
			...
			if(!M("mod_b2b_shop")->selectRow("shopname='".$barr[0]."'")){
	...
	public function onShop_product_category(){
		$shopList=M("mod_b2b_shop")->select(array());
		$cats=array(
			"吃的",
			"穿的",
			"用的",
		...
		M("mod_b2b_shop_product_category")->delete("1");
		foreach($shopList as $shop){
			shuffle($cats);
			foreach($cats as $k=>$cat){
				M("mod_b2b_shop_product_category")->insert(array(
					"title"=>$cat,
					"shopid"=>$shop["shopid"],
					"status"=>1
				));
	...
	public function onShop_product(){
		...
		$table="mod_b2b_product";
		$c=file_get_contents($file);
		$res=json_decode($c,true);
		M($table)->delete("1");
		$mcids=M("mod_b2b_category")->selectCols(array(
			"fields"=>"catid",
			"where"=>" status=1 "
		));
		foreach($shopList as $shop){
			$catids=M("mod_b2b_shop_product_category")->selectCols(array(
				"fields"=>"catid",
	...
	public function onGroup(){
		M("mod_b2b_group")->delete("1");
		
		$gid=M("mod_b2b_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"recommend",
			"gnum"=>12,
			"status"=>1
		));
		$gid2=M("mod_b2b_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"hot",
			"gnum"=>12,
			"status"=>1
		));
		$list=M("mod_b2b_product")->selectCols(array(
			"where"=>" status=1 ",
			"limit"=>24
		));

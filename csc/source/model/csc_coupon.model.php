<?php
class csc_couponModel extends model{
	public $base;
	public function __construct(){
		parent::__construct();
		$this->base=$base;
		$this->table="mod_csc_coupon";
	}
	...
	public function listByShop($userid,$shopid){
		...
		$da=M("mod_csc_coupon_user")->select(array("where"=>$where));
		if($da){
			foreach($da as $v){
				$ids[]=$v['coupon_id'];
			}
	...
	public function UseList($shopid,$userid,$money){
		...
		//$list=M("mod_csc_coupon")->select(array("where"=>$where));
		//已获取的券
		$where=" status=0 AND shopid=".$shopid."  AND userid=".$userid;
		$da=M("mod_csc_coupon_user")->select(array("where"=>$where));
		$coupons=array();

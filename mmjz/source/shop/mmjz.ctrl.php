...
class mmjzControl extends skymvc{
	...
	public function onDefault(){
		...
		$admin=M("mod_mmjz_admin")->selectRow("adminid=".$_SESSION["mmmjz_shop_admin"]["adminid"]);
		$timeago=timeago(strtotime($admin["xlasttime"]));
		$neworder=M("mod_mmjz_order")->selectOne(array(
			"where"=>" ispay=1 AND status=0 AND shopid=".SHOPID,
			"fields"=>" count(*) as ct "
		));
		$dayorder=M("mod_mmjz_order")->selectRow(array(
		...
		$usernum=M("mod_mmjz_order")->selectCols(array(
			"where"=>" shopid=".SHOPID,
			"fields"=>"userid",
			"limit"=>1000000
		));
		//橱窗商品
		$prolist=MM("mmjz","mmjz_product")->Dselect(array(
	...
	public function onNotice(){
		$neworder=M("mod_mmjz_order")->selectRow("ispay=1 AND status=0 AND shopid=".SHOPID." ");
		$ctime=date("Y-m-d H:i:s",time()-36000);
		$newMsg=M("mod_mmjz_guest")->selectRow("status=1 AND ukey='shop' AND shopid=".SHOPID." AND createtime>'".$ctime."' ");
		$data=array(

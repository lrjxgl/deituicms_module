...
class csc_orderControl extends skymvc{
	...
	public function onConfirm(){
		...
		$shopid=get("shopid","i");
		$shop=MM("csc","csc_shop")->get($shopid,"shopid,express_money,shopname,imgurl,yystatus,yystart,yyend");
		if(empty($shop)){
			$this->goAll("请选择商家",1);
		}
		...
		$start=explode(":",$shop["yystart"]);
		$stime=$start[0]*3600+$start[1]*60;
		$end=explode(":",$shop["yyend"]);
		$etime=$end[0]*3600+$end[1]*60;
		$ntime=intval(date("H")) *3600 +intval(date("i"))*60;
		...
		//优惠券
		$coupon_id=0;
		$couponList=MM("csc","csc_coupon")->UseList($shopid,$userid,$total_money);
		if($couponList){
		...
		$express_money=$shop["express_money"];
		//会员
		$vip=MM("csc","csc_vip_user")->get($userid,SHOPID);
		if($vip){
			$express_money=0;
			if($vip["vipid"]==3){
				$vday=M("mod_csc_vip_day")->selectRow(
					array(
						"where"=>array(
							"userid"=>$userid,
							"shopid"=>SHOPID,
				...
				if(!$vday){
					$total_money=$total_money-$vip["discount"];
					$total_money=max(0,$total_money);
				}
				
			}else{
				$total_money=intval($total_money*$vip["discount"]*100)/10000;
			}
		...
		$this->smarty->display("csc_order/confirm.html");
	}
	public function onBuy(){
		
		$this->smarty->display("csc_order/buy.html");
	}
	public function onOrder(){
		...
		if($coupon_id){
			...
			$coupon=M("mod_csc_coupon")->selectRow("id=".$coupon_id);
			$etime=strtotime($coupon['end_time']);
			if(!$coupon || $goods_money<$coupon['lower_money'] || $etime<time()){
				//失效
			}else{
				$coupon_user=M("mod_csc_coupon_user")->selectRow("coupon_id=".$coupon_id." AND status=0 AND userid=".$userid);			
				if($coupon_user || $coupon["typeid"]==1){
					$coupon_money=$coupon['money'];
					if($coupon_user){
						M("mod_csc_coupon_user")->update(array("status"=>1),"id=".$coupon_user['id']);
					}				
					M("mod_csc_coupon")->update(array("use_num"=>$coupon['use_num']+1),"id=".$coupon_id);
		...
		if($vip){
			...
			if($vip["vipid"]==3){
				...
				if(!$vday){
					$total_money=$total_money-$vip["discount"];
					$total_money=max(0,$total_money);
					M("mod_csc_vip_day")->insert(array(
							"userid"=>$userid,
							"shopid"=>SHOPID,
							"vipid"=>$vip["vipid"],
							"cday"=>date("Y-m-d")
		...
		if($user["money"]>=$total_money){
			...
			M("user")->addMoney(array(
				"userid"=>$userid,
				"money"=>-$total_money,
				"content"=>"您在商城购买商品,花了{$total_money}元"
			));	
				
		}
		$orderid=MM("csc","csc_order")->insert(array(
			"shopid"=>$shopid,
			"money"=>$total_money,
			"goods_money"=>$goods_money,
			"express_money"=>$express_money,
		...
		foreach($cartList as $v){
			...
			$prolist[]=$v;
			M("mod_csc_order_product")->insert(array(
				"shopid"=>$shopid,
				"orderid"=>$orderid,
				"createtime"=>$createtime,
				"userid"=>$userid,
				...
				"amount"=>$v["amount"],
				"supid"=>$v["supid"],
				"sup_title"=>$v["sup_title"]
			));
		}
		//生成订单地址
		M("mod_csc_order_address")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"createtime"=>$createtime,
			"userid"=>$userid,
		...
		M("mod_csc_order_data")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"userid"=>$userid,
			"createtime"=>date("Y-m-d H:i:s"),
		...
		foreach($cartList as $v){
			if($v["ksid"]){
				MM("csc","csc_product_ks")->changenum("total_num",-$v["amount"],"id=".$v["ksid"]);
			}else{
				MM("csc","csc_product")->changenum("total_num",-$v["amount"],"id=".$v["productid"]);
			}
		}	
		//清除购物车
		MM("csc","csc_cart")->delete(" id in("._implode($cartids).")");
		...
		if(!$ispay){
			...
			/**发送通知**/
			MM("csc","csc_shop_notice")->sendNewOrder($orderid);
			$rdata["action"]="finish";
		}
	...
	public function onPay($return=0){
		...
		if($backurl==""){
			$backurl="/module.php?m=csc_order&a=success";
		}
	...
	public function onSuccess(){
		$this->smarty->display("csc_order/success.html");
	}
	public function onMy(){
		$userid=M("login")->userid;
		$where=" userid=".$userid;
		$url="/module.php?m=csc_order&a=my";
		$limit=12;
		...
		$this->smarty->display("csc_order/my.html");
	}
	public function onShow(){
		$orderid=get("orderid","i");
		$order=MM("csc","csc_order")->selectRow("orderid=".$orderid);
		$orderdata=MM("csc","csc_order_data")->get($orderid);
		$shop=MM("csc","csc_shop")->get($order["shopid"]); 
		...
		if($order["ispay"] && $order["status"]<3){
			$ordercode=MM("csc","csc_order_code")->get($orderid,$order["shopid"]);
			$c=json_encode(array(
				"action"=>"url",
				"url"=>"../csc_order_code/index?ordercode=".$ordercode
			));
	...
	public function onRaty(){
		...
		$sql="select a.id,a.raty_grade,a.productid,b.title,b.imgurl from ".table("mod_csc_order_product")." as a "
			." left join ".table("mod_csc_product")." as b "
			." on a.productid=b.id "
			." where a.orderid=".$orderid;
		$proList=MM("csc","csc_order")->getAll($sql);
		if($proList){
		...
		$raty=MM("csc","mod_csc_order_raty")->selectRow("orderid=".$orderid);
		if(empty($raty)){
			$raty=array(
				"raty_quality"=>9,
				"raty_service"=>9,
		...
		$this->smarty->display("csc_order/raty.html");
	}

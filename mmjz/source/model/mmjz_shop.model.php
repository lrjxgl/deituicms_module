<?php
class mmjz_shopModel extends model{
	public $table="mod_mmjz_shop";
	...
	public function DselectWindow($option=array(),&$rscount=false){
		...
		if($data){
			...
			//优惠券
			$cps=MM("mmjz","mmjz_coupon")->select(array(
	...
	public function vipList($limit=5){
		$res=M("mod_mmjz_shop_hotvip")->select(array(
			"where"=>"isfinish=0",
			"order"=>"spid DESC",
			"fields"=>"shopid,spid",
			"limit"=>100
		...
		if(!empty($shopids)){
			...
			$list=MM("mmjz","mmjz_shop")->DselectWindow(array(

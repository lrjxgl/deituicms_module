...
class wmo2oControl extends skymvc{
	...
	public function onDefault(){
		...
		$admin=M("mod_wmo2o_admin")->selectRow("adminid=".$_SESSION["mwmo2o_shop_admin"]["adminid"]);
		$timeago=timeago(strtotime($admin["xlasttime"]));
		$neworder=M("mod_wmo2o_order")->selectOne(array(
			"where"=>" ispay=1 AND status=0 AND shopid=".SHOPID,
			"fields"=>" count(*) as ct "
		));
		$dayorder=M("mod_wmo2o_order")->selectRow(array(
		...
		$usernum=M("mod_wmo2o_order")->selectCols(array(
			"where"=>" shopid=".SHOPID,
			"fields"=>"userid",
			"limit"=>1000000
		));
		//橱窗商品
		$prolist=MM("wmo2o","wmo2o_product")->Dselect(array(
	...
	public function onNotice(){
		$neworder=M("mod_wmo2o_order")->selectRow("ispay=1 AND status=0 ");
		$ctime=date("Y-m-d H:i:s",time()-36000);
		$newMsg=M("mod_wmo2o_guest")->selectRow("status=1 AND ukey='shop' AND createtime>'".$ctime."' ");
		$data=array(

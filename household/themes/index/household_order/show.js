$(document).on("click","#order-cancel-submit",function(){
	if(confirm("确认取消订单?")){
		$.get("/module.php?m=household_order&a=cancel&ajax=1&orderid="+orderid,function(res){
...
$(document).on("click","#order-receive-submit",function(){
	$.get("/module.php?m=household_order&a=receive&ajax=1&orderid="+orderid,function(res){
		window.location.reload();
	})
})

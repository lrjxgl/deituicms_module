<?php
class fenlei_tagsModel extends model{
	public $table="mod_fenlei_tags";
	...
	public function Done($objectid,$tags){
		...
		$indexlist=M("mod_fenlei_tags_index")->selectCols(array(
		...
		if($deleteIndex){
			...
			M("mod_fenlei_tags_index")->delete("objectid=".$objectid." AND tagid in("._implode($delTagsId).") ");
		...
		if(!empty($insertIndex)){
			M("mod_fenlei_tags_index")->insertMore($insertIndex);
		}
		 
	}
	
	public function getfenleiByKey($gkey){
		$group=$this->selectRow(array(
			"where"=>" gkey='".sql($gkey)."' ",
			 "fields"=>"tagid,gnum"
		));
		if(!$group) return false;
		$data=M("mod_fenlei_tags_index")->select(array(
		...
		if($data){
			...
			$pros=MM("fenlei","fenlei")->getListByIds($proids);
			foreach($data as $k=>$v){
				$v=$pros[$v["objectid"]];
				$data[$k]=$v;
			}

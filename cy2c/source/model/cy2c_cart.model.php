<?php
	class cy2c_cartModel extends model{
		public $table="mod_cy2c_cart";
		...
		public function Dselect($option=array()){
			...
			if($res){
				...
				$kss=MM("cy2c","cy2c_product_ks")->getListByIds($ksids);
				foreach($res as $k=> $rs){
		...
		public function getProductAmount($where){
			$list=MM("cy2c","cy2c_cart")->select(array("where"=>$where));
			if($list){
				foreach($list as $v){

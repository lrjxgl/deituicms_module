...
class jdo2o_blogControl extends skymvc{
	...
	public function onShow(){
		...
		$data=MM("jdo2o","jdo2o_blog")->selectRow("id=".$id." AND status in(0,1) ");
		if(!$data) $this->goAll("数据出错",1);
		$data["mp4url"]=images_site($data["mp4url"]);
		//浏览记录
		if($userid){
			$view=M("mod_jdo2o_blog_view")->selectRow("userid=".$userid." AND objectid=".$id);
			if(!$view){
				M("mod_jdo2o_blog_view")->insert(array(
					"userid"=>$userid,
					"objectid"=>$id,
					"createtime"=>date("Y-m-d H:i:s")
				));
				MM("jdo2o","jdo2o_blog")->update(array(
		...
		//是否点赞
		$islove=0;
		$love=M("love")->selectRow("tablename='mod_jdo2o_blog' AND userid=".$userid." AND objectid=".$id);
		if($love){
			$islove=1;
		}
		//是否收藏
		$isfav=0;
		if($userid){
			$fav=M("fav")->selectRow("tablename='mod_jdo2o_blog' AND userid=".$userid." AND objectid=".$id);
			if($fav){
				$isfav=1;
			}
		}
		$this->smarty->goAssign(array(
			...
			"data"=>$data,
			"comment_objectid"=>$id,
			"comment_tablename"=>"mod_jdo2o_blog",
			"comment_f_userid"=>$data['userid'],
			"imgslist"=>$imgslist,
			"author"=>$author,
			"userid"=>$userid
		));
		$this->smarty->display("jdo2o_blog/show.html");
	}
	...
	public function onMy(){
		...
		$this->smarty->display("jdo2o_blog/my.html");
	}
	...
	public function onAdd(){
		...
		$this->smarty->display("jdo2o_blog/add.html");
	}
	...
	public function onDelete(){
		...
		M("mod_jdo2o_blog")->update(array("status"=>11),"id=".$id);
		$this->goAll("删除成功");
	}

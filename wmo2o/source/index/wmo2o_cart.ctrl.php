<?php
	class wmo2o_cartControl extends skymvc{
		...
		public function onDefault(){
			...
			$this->smarty->display("wmo2o_cart/index.html");
		}
		public function onAdd(){
			...
			$ksid=get("ksid","i");
			$product=MM("wmo2o","wmo2o_product")->selectRow("id=".$productid);
			...
			$amount=get("amount","i");
			$cart=MM("wmo2o","wmo2o_cart")->selectRow("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
			$ks=MM("wmo2o","wmo2o_product_ks")->selectRow("id=".$ksid);
			if($ks){
			...
			$cart=MM("wmo2o","wmo2o_cart")->selectRow("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
			if($amount<=0){
				MM("wmo2o","wmo2o_cart")->delete("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
				$rdata=array(
					"action"=>"delete",
					"ksid"=>$ksid,
					"productid"=>$productid,
				...
				MM("wmo2o","wmo2o_cart")->update(array(	
				...
				$cart["id"]=MM("wmo2o","wmo2o_cart")->insert(array(	
					"createtime"=>date("Y-m-d H:i:s"),
					"amount"=>$amount,
					"ksid"=>$ksid,
					"productid"=>$productid,
		...
		public function onClear(){
			$userid=M("login")->userid;
			MM("wmo2o","wmo2o_cart")->delete("userid=".$userid);
			$this->goAll("清除成功");
		}

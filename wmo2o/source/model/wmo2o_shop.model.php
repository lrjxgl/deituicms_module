<?php
class wmo2o_shopModel extends model{
	public $table="mod_wmo2o_shop";
	...
	public function DselectWindow($option=array(),&$rscount=false){
		...
		if($data){
			...
			//优惠券
			$cps=MM("wmo2o","wmo2o_coupon")->select(array(
	...
	public function vipList($limit=5){
		$res=M("mod_waimai_shop_hotvip")->select(array(
			"where"=>"isfinish=0",
			"order"=>"spid DESC",
			"fields"=>"shopid,spid",
			"limit"=>100
		...
		if(!empty($shopids)){
			...
			$list=MM("waimai","waimai_shop")->DselectWindow(array(

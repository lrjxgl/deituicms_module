<?php
	class f2c_cartModel extends model{
		public $table="mod_f2c_cart";
		...
		public function Dselect($option=array()){
			...
			if($res){
				...
				$pros=MM("f2c","f2c_product")->getListByIds($productids);
				$kss=MM("f2c","f2c_product_ks")->getListByIds($ksids);
				foreach($res as $k=> $rs){
		...
		public function getProductAmount($where){
			$list=MM("f2c","f2c_cart")->select(array("where"=>$where));
			if($list){
				foreach($list as $v){

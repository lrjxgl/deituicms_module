...
class wmo2o_shop_earnestControl extends skymvc{
	...
	public function onDefault(){
		...
		$url="/moduleadmin.php?m=wmo2o_shop_earnest";
		...
		if($shopname){
			$shopids=MM("wmo2o","wmo2o_shop")->selectCols(array(
		...
		$rscount=true; 
		$list=MM("wmo2o","wmo2o_shop_earnest")->select($ops,$rscount);
		if(!empty($list)){
			$shopids=[];
			foreach($list as $v){
				$shopids[]=$v["shopid"];
	...
	public function onSave(){
		...
		$sp=MM("wmo2o","wmo2o_shop_earnest")->get($shopid);
		if($sp["money"]<$money){
			$this->goAll("商家保证金不足",1);
		}
		//减少余额
		MM("wmo2o","wmo2o_shop_earnest")->begin();
		MM("wmo2o","wmo2o_shop_earnest")->addMoney(array(
			"shopid"=>$shopid,
			"money"=>-$money,
			"content"=>$content
		)); 
		MM("wmo2o","wmo2o_shop_earnest")->commit();
		$this->goAll("扣除成功");
	}

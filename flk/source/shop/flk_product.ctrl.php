<?php
	...
	*/
	...
	function onWindow(){	 
		...
		$this->smarty->display("flk_product/window.html");
	}
	...
	public function onWindowSave(){
		...
		if($row['iswindow']){
			...
			$ct=M("mod_flk_product")->selectOne(array(
		...
		public function onAdd(){
			...
			if($id){
				...
				if($data["one_status"]!=0 || $data["one_status"]==1){
					$this->goAll("活动期间该产品无法编辑",1);
				}
			...
			$shop_catlist=MM("flk","flk_shop_product_category")->children(SHOPID,0);
			//增加扩展表
			$fieldsList=array();
			if($cat){
		...
		public function onSave(){
			...
			$data=M("mod_flk_product")->postData($unpost);
			...
			if($id){
				$row=M("mod_flk_product")->selectRow("id=".$id);
				if($row["one_status"]!=0 || $row["one_status"]==1){
					$this->goAll("活动期间该产品无法编辑",1);
				}
				if($row["shopid"]!=SHOPID){
					$this->goAll("暂无权限",1);

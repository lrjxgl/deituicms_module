...
var app = new Vue({
	...
	methods: {
		goProduct:function(id){
			window.location="/module.php?m=gxny_product&a=show&id="+id;
		},
		getPage: function() {
			var that = this;
			$.get("/module.php?m=gxny_product&ajax=1", function(res) {
		...
		getList:function(){
			var that=this;
			$.ajax({
				url:"/module.php?m=gxny_product&ajax=1",
				data:{
					catid:catid
				},
				dataType:"json",
		...
		addCart: function(id, ksid) {
			...
			var amount = 1;
			$.ajax({
				url: '/module.php?m=gxny_cart&a=add&ajax=1',
				data: {
					productid: id,
					amount: amount,
					ksid: ksid

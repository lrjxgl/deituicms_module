$(document).on("click","#order-cancel-submit",function(){
	if(confirm("确认取消订单?")){
		$.get("/moduleshop.php?m=pdd_order&a=cancel&ajax=1&orderid="+orderid,function(res){
			window.location.reload();
		})
	}
})
$(document).on("click","#order-receive-submit",function(){
	$.get("/moduleshop.php?m=pdd_order&a=receive&ajax=1&orderid="+orderid,function(res){
		window.location.reload();
	})
})
...
$(document).on("click","#goPay",function(){
	...
	$.ajax({
		url:"/moduleshop.php?m=pdd_order&a=pay&ajax=1&orderid="+orderid,
		dataType:"json",

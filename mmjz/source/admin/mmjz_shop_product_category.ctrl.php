	...
	*/
	...
	class mmjz_shop_product_categoryControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=mmjz_shop_product_category&a=default";
			$limit=20;
			...
			$rscount=true;
			$data=M("mod_mmjz_shop_product_category")->select($option,$rscount);
			if($data){
				foreach($data as $v){
					$shopids[]=$v["shopid"];
				}
		...
		public function onStatus(){
			...
			M("mod_mmjz_shop_product_category")->update(array("status"=>$status),"catid=$catid");
			$this->goall("状态修改成功");
		}

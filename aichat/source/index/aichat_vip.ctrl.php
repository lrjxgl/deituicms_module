...
class aichat_vipControl extends skymvc{
	...
	public function onBuy(){
		M("login")->checkLogin();
		$userid=M("login")->userid;
		$list=M("mod_aichat_vip")->select(array(
			"where"=>" status=1 ",
		...
		$this->smarty->display("aichat_vip/buy.html");
	}
	public function onSave(){
		...
		M("mod_aichat_vip_order")->insert(array(
			"userid"=>$userid,
			"tokenid"=>$row["tokenid"],
			"num"=>$row["num"],
			"money"=>$row["price"],
	...
	public function onPay($return=0){
		...
		$order=MM("aichat","aichat_vip_order")->selectRow("orderid=".$orderid);
		//生成支付
		...
		$orderdata=array(
			"table"=>"plugin",
			"callback"=>'
				MM("aichat","aichat_vip_order")->buySuccess(array(
					"ispay"=>1,

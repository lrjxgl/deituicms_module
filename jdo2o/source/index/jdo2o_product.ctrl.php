	...
	*/
	...
	class jdo2o_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/module.php?m=jdo2o_product&a=default";
			$limit=20;
			...
			//分类
			$catList=MM("jdo2o","jdo2o_shop_product_category")->children($shopid);
			//判断产品是否在购物车
			$userid=M("login")->userid;
			$cartPros=MM("jdo2o","jdo2o_cart")->getProductAmount("userid=".$userid." AND shopid=".$shopid." AND ksid=0");
			...
			$express_money=MM("jdo2o","jdo2o_express_fee")->getMoney($user_address_id,$weight,$shopid,$shop["express_money"]);
			if($data){
				foreach($data as $k=>$v){
					$v["incart"]=0;
					$v["cart_amount"]=0;
		...
		public function onList(){
			...
			$catid=get("catid","i");
			if($catid){
				$cids=MM("jdo2o","jdo2o_category")->id_family($catid);
			...
			$data=M("mod_jdo2o_product")->select($option,$rscount);
			//分类
			$cat=MM("jdo2o","jdo2o_category")->selectRow(array(
				"where"=>"catid=".$catid,
				"fields"=>"catid,title,pid"
			));
			$catList=MM("jdo2o","jdo2o_category")->children($catid);
			
			 
			
			//判断产品是否在购物车
			$userid=M("login")->userid;
			$cartPros=MM("jdo2o","jdo2o_cart")->getProductAmount("userid=".$userid." AND ksid=0");
			if($data){
				foreach($data as $k=>$v){
					$v["incart"]=0;
					$v["cart_amount"]=0;
			...
			$this->smarty->display("jdo2o_product/list.html");
		}
		
		public function onShow(){
			$id=get_post("id","i");
			$data=MM("jdo2o","jdo2o_product")->selectRow(array("where"=>"id=".$id));
			...
			$ksList=MM("jdo2o","jdo2o_product_ks")->getListByTitle($id);
			if($ksList){
				$ks=$ksList[0];
				$ksid=$ks["id"];
			...
			$cart_amount=0;
			$userid=M("login")->userid;
			$cart=MM("jdo2o","jdo2o_cart")->selectRow("userid=".$userid." AND productid=".$id." AND ksid=".$ksid);
			if($cart){
				$cart_amount=$cart["amount"];
			}
			$shopid=$data["shopid"];
			$shop=MM("jdo2o","jdo2o_shop")->get($shopid,"shopid,shopname,imgurl,raty_grade,buy_num");
			...
			$this->smarty->display("jdo2o_product/show.html");
		}

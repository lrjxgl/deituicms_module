<?php
class tutor_tixianControl extends skymvc{
	public $sw;
	public $k;
	public function __construct(){
		parent::__construct();	
		$this->k="mod_tutor_shop";
	...
	public function onList(){
		$limit=20;
		$where= $this->sw;
		$url="/moduleshop.php?m=tutor_tixian";
		...
		$this->smarty->display("tutor_tixian/list.html");
	}
	
	
	public function onDefault(){
		$shop=M("mod_tutor_shop")->selectRow("shopid=".SHOPID);
		$shopmoney=MM("tutor","tutor_shop_money")->get(SHOPID);
		$last=M("tixian")->selectRow(array(
			"order"=>"id DESC",
			"where"=>$this->sw,
			"limit"=>1
		));
		$bankList=M("mod_tutor_bankcard")->select(array(
			"where"=>"shopid=".SHOPID
		));	
		$this->smarty->goassign(array(
			"shop"=>$shop,
		...
		$this->smarty->display("tutor_tixian/index.html");
	}
	...
	public function onSave(){
		...
		$shop=M("mod_tutor_shop_money")->selectRow("shopid=".SHOPID);
		...
		$bank=M("mod_tutor_bankcard")->selectRow("id=".$bankid);

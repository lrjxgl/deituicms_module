$(document).on("click","#order-cancel-submit",function(){
	if(confirm("确认取消订单?")){
		$.get("/module.php?m=cy2c_order&a=cancel&ajax=1&orderid="+orderid,function(res){
...
$(document).on("click","#order-receive-submit",function(){
	$.get("/module.php?m=cy2c_order&a=receive&ajax=1&orderid="+orderid,function(res){
		window.location.reload();
	})
})
...
$(document).on("click","#goPay",function(){
	...
	$.ajax({
		url:"/module.php?m=cy2c_order&a=pay&ajax=1&orderid="+orderid,
		dataType:"json",

	...
	*/
	...
	class cy2c_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=cy2c_product&a=default";
			$limit=20;
			...
			if($data){
				...
				$cats=MM("cy2c","cy2c_category")->getListByIds($cids);
				foreach($data as $k=>$v){
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_cy2c_product")->selectRow(array("where"=>"id=".$id));
			...
			$this->smarty->display("cy2c_product/add.html");
		}
		
		public function onSave(){
			$id=get_post("id","i");
			$data=M("mod_cy2c_product")->postData();
			$content=post("content","x");
			$sdata=array(
				"content"=>$content
			);
			if($id){
				M("mod_cy2c_product")->update($data,"id='$id'");
				M("mod_cy2c_product_data")->update($sdata,"id='$id'");
			}else{
				$id=M("mod_cy2c_product")->insert($data);
				$sdata["id"]=$id;
				M("mod_cy2c_product_data")->insert($sdata);
			}
			$this->goall("保存成功");
		}
		...
		public function onStatus(){
			$id=get_post('id',"i");
			$row=M("mod_cy2c_product")->selectRow("id=".$id);
			$status=1;
			if($row["status"]==1){
				$status=2;
			}
			M("mod_cy2c_product")->update(array(
				"status"=>$status
			),"id=".$id);
		...
		public function onDelete(){
			$id=get_post('id',"i");
			M("mod_cy2c_product")->update(array("status"=>11),"id=$id");
			$this->goAll("删除成功");
		...
		public function onCategory(){
			...
			if($ids){
				foreach($ids as $id){
					M("mod_cy2c_product")->update(array("catid"=>$catid),"id=".$id);
				}
			}
			$this->goall("修改成功");
		}

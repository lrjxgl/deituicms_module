...
$(document).on("click", "#ks1 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=b2b_product_ks&a=sizeList&ajax=1&id=" + kid,
		dataType: "json",
...
$(document).on("click", "#ks2 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=b2b_product_ks&a=get&ajax=1&id=" + kid,
		dataType: "json",
		success: function(res) {
			that.addClass("kslist-active").siblings().removeClass("kslist-active");
			ksid = res.data.ksid;
	...
	$.ajax({
		...
		success: function(res) {
			...
			if(goBuy){
				...
				window.location="/module.php?m=b2b_order&a=confirm&shopid="+shopid+"&cartid="+res.data.cartid
			}

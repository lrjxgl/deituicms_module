	...
	*/
	...
	class f2c_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$catid=get("catid","i");
			if($catid){
				$cids=MM("f2c","f2c_category")->id_family($catid);
			...
			$cartPros=MM("f2c","f2c_cart")->getProductAmount("userid=".$userid." AND ksid=0");
			if($data){
				foreach($data as $k=>$v){
					$v["incart"]=0;
					$v["cart_amount"]=0;
		...
		public function onList(){
			...
			$data=M("mod_f2c_product")->select($option,$rscount);
			//分类
			$cat=MM("f2c","f2c_category")->selectRow(array(
				"where"=>"catid=".$catid,
				"fields"=>"catid,title,pid"
			));
			$catList=MM("f2c","f2c_category")->children($catid);
			...
			$this->smarty->display("f2c_product/list.html");
		}
		
		public function onShow(){
			$id=get_post("id","i");
			$data=MM("f2c","f2c_product")->selectRow(array("where"=>"id=".$id));
			...
			$ksList=MM("f2c","f2c_product_ks")->getListByTitle($id);
			if($ksList){
				$ks=$ksList[0];
				$ksid=$ks["id"];
				$ksList2=MM("f2c","f2c_product_ks")->select(array(
			...
			$cart_amount=0;
			$userid=M("login")->userid;
			$cart=MM("f2c","f2c_cart")->selectRow("userid=".$userid." AND productid=".$id." AND ksid=".$ksid);
			if($cart){
			...
			$this->smarty->display("f2c_product/show.html");
		}
		...
		public function onRaty(){
			...
			$start=get("per_page","i");
			$rscount=true;
			$list=M("mod_f2c_order_product")->select(array(
				"where"=>$where,
				"limit"=>$limit,
				"start"=>$start,
				"order"=>" id DESC"
			...
			$this->smarty->display("f2c_product/raty.html");
		}

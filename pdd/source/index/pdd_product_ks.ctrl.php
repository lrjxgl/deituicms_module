...
class pdd_product_ksControl extends skymvc{
	...
	public function onDefault(){
		...
		$ksList=MM("pdd","pdd_product_ks")->getListByTitle($productid);
		$ksList2=array();
		$ksid=0;
		if($ksList){
			$ks=$ksList[0];
		...
		if($userid){
			$cart=MM("pdd","pdd_cart")->selectRow("userid=".$userid." AND ksid=".$ksid);
			if($cart){
				$product["incart"]=1;
	...
	public function onsizeList(){
		...
		$ksid=$row["id"];
		$ksList=M("mod_pdd_product_ks")->select(array(

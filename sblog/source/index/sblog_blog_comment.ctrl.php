...
class sblog_blog_commentControl extends skymvc{
	...
	public function onSave(){
		...
		$data=M("mod_sblog_blog_comment")->postData();
		...
		$row=M("mod_sblog_blog")->selectRow(array(
			"where"=>" id=".$data['objectid'],			 
		));
		$data["userid"]=$userid;
		M("mod_sblog_blog_comment")->insert($data);
		M("mod_sblog_blog")->changenum("comment_num",1,"id=".$data["objectid"]);
		if($row["topicid"]){
			M("mod_sblog_topic")->changenum("comment_num",1,"id=".$row["topicid"]);
		}
		M("notice")->add(array(

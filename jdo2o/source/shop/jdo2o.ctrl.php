...
class jdo2oControl extends skymvc{
	...
	public function onDefault(){
		...
		$usernum=M("mod_jdo2o_order")->selectCols(array(
			"where"=>" shopid=".SHOPID,
			"fields"=>"userid",
			"limit"=>1000000
		));
		//橱窗商品
		$prolist=MM("jdo2o","jdo2o_product")->Dselect(array(
	...
	public function onNotice(){
		$neworder=M("mod_jdo2o_order")->selectRow("ispay=1 AND status=0 ");
		$ctime=date("Y-m-d H:i:s",time()-36000);
		$newMsg=M("mod_jdo2o_guest")->selectRow("status=1 AND ukey='shop' AND createtime>'".$ctime."' ");
		$data=array(

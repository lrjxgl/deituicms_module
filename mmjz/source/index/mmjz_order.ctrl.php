...
class mmjz_orderControl extends skymvc{
	...
	public function onConfirm(){
		...
		$shopid=get("shopid","i");
		$shop=MM("mmjz","mmjz_shop")->get($shopid,"shopid,shopname,imgurl,yystatus");
		if(empty($shop)){
			$this->goAll("请选择商家",1);
		}
		if($shop["yystatus"]!=1){
		...
		//优惠券
		$coupon_id=0;
		$couponList=MM("mmjz","mmjz_coupon")->UseList($shopid,$userid,$total_money);
		if($couponList){
		...
		$this->smarty->display("mmjz_order/confirm.html");
	}
	public function onBuy(){
		
		$this->smarty->display("mmjz_order/buy.html");
	}
	public function onOrder(){
		...
		if($coupon_id){
			...
			$coupon=M("mod_mmjz_coupon")->selectRow("id=".$coupon_id);
			$etime=strtotime($coupon['end_time']);
			if(!$coupon || $goods_money<$coupon['lower_money'] || $etime<time()){
				//失效
			}else{
				$coupon_user=M("mod_mmjz_coupon_user")->selectRow("coupon_id=".$coupon_id." AND status=0 AND userid=".$userid);			
				if($coupon_user || $coupon["typeid"]==1){
					$coupon_money=$coupon['money'];
					if($coupon_user){
						M("mod_mmjz_coupon_user")->update(array("status"=>1),"id=".$coupon_user['id']);
					}				
					M("mod_mmjz_coupon")->update(array("use_num"=>$coupon['use_num']+1),"id=".$coupon_id);
		...
		$orderid=MM("mmjz","mmjz_order")->insert(array(
			"shopid"=>$shopid,
			"money"=>$total_money,
			"goods_money"=>$goods_money,
			"express_money"=>$express_money,
		...
		foreach($cartList as $v){
			$prolist[]=$v;
			M("mod_mmjz_order_product")->insert(array(
				"shopid"=>$shopid,
				"orderid"=>$orderid,
				"createtime"=>$createtime,
				"userid"=>$userid,
		...
		//生成订单地址
		M("mod_mmjz_order_address")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"createtime"=>$createtime,
			"userid"=>$userid,
		...
		M("mod_mmjz_order_data")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"userid"=>$userid,
			"createtime"=>date("Y-m-d H:i:s"),
		...
		foreach($cartList as $v){
			if($v["ksid"]){
				MM("mmjz","mmjz_product_ks")->changenum("total_num",-$v["amount"],"id=".$v["ksid"]);
			}else{
				MM("mmjz","mmjz_product")->changenum("total_num",-$v["amount"],"id=".$v["productid"]);
			}
		}	
		//清除购物车
		MM("mmjz","mmjz_cart")->delete(" id in("._implode($cartids).")");
		...
		if(!$data['ispay']){
			...
			/**发送通知**/
			MM("mmjz","mmjz_shop_notice")->sendNewOrder($orderid);
			$rdata["action"]="finish";
		}
	...
	public function onPay($return=0){
		...
		if($backurl==""){
			$backurl="/module.php?m=mmjz_order&a=success";
		}
	...
	public function onSuccess(){
		$this->smarty->display("mmjz_order/success.html");
	}
	public function onMy(){
		$userid=M("login")->userid;
		$where=" userid=".$userid;
		$url="/module.php?m=mmjz_order&a=my";
		$limit=24;
		...
		$this->smarty->display("mmjz_order/my.html");
	}
	public function onShow(){
		$orderid=get("orderid","i");
		$order=MM("mmjz","mmjz_order")->selectRow("orderid=".$orderid);
		$orderdata=MM("mmjz","mmjz_order_data")->get($orderid);
		$shop=MM("mmjz","mmjz_shop")->get($order["shopid"]); 
		...
		if($order["ispay"] && $order["status"]<3){
			$ordercode=MM("mmjz","mmjz_order_code")->get($orderid,$order["shopid"]);
			$c=json_encode(array(
				"action"=>"url",
				"url"=>"../mmjz_order_code/index?ordercode=".$ordercode
			));
	...
	public function onRaty(){
		...
		$sql="select a.id,a.raty_grade,a.productid,b.title,b.imgurl from ".table("mod_mmjz_order_product")." as a "
			." left join ".table("mod_mmjz_product")." as b "
			." on a.productid=b.id "
			." where a.orderid=".$orderid;
		$proList=MM("mmjz","mmjz_order")->getAll($sql);
		if($proList){
		...
		$raty=MM("mmjz","mod_mmjz_order_raty")->selectRow("orderid=".$orderid);
		if(empty($raty)){
			$raty=array(
				"raty_quality"=>9,
				"raty_service"=>9,
		...
		$this->smarty->display("mmjz_order/raty.html");
	}

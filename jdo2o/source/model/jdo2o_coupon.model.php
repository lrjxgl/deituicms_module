<?php
class jdo2o_couponModel extends model{
	 
	public function __construct(){
		parent::__construct();
		 
		$this->table="mod_jdo2o_coupon";
	}
	...
	public function listByShop($userid,$shopid){
		...
		$da=M("mod_jdo2o_coupon_user")->select(array("where"=>$where));
		if($da){
			foreach($da as $v){
				$ids[]=$v['coupon_id'];
			}
	...
	public function UseList($shopid,$userid,$money){
		...
		$list=M("mod_jdo2o_coupon")->select(array("where"=>$where));

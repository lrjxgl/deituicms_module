<?php
	...
	*/
	...
	class b2b_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/module.php?m=b2b_product&a=default";
			$limit=20;
			...
			$rscount=true;
			$data=M("mod_b2b_product")->select($option,$rscount);
			//分类
			$catList=MM("b2b","b2b_shop_product_category")->children($shopid);
			//判断产品是否在购物车
			$userid=M("login")->userid;
			$cartPros=MM("b2b","b2b_cart")->getProductAmount("userid=".$userid." AND shopid=".$shopid." AND ksid=0");
			
			$cartList=MM("b2b","b2b_cart")->Dselect(array(
			...
			$express_money=MM("b2b","b2b_express_fee")->getMoney($user_address_id,$weight,$shopid,$shop["express_money"]);
			if($data){
				foreach($data as $k=>$v){
					$v["incart"]=0;
					$v["cart_amount"]=0;
			...
			//推荐商品
			$recList=MM("b2b","b2b_product")->Dselect(array(
			...
			$this->smarty->display("b2b_product/index.html");
		}
		...
		public function onList(){
			...
			$catid=get("catid","i");
			if($catid){
				$cids=MM("b2b","b2b_category")->id_family($catid);
			...
			$data=M("mod_b2b_product")->select($option,$rscount);
			//分类
			$cat=MM("b2b","b2b_category")->selectRow(array(
				"where"=>"catid=".$catid,
				"fields"=>"catid,title,pid"
			));
			$catList=MM("b2b","b2b_category")->children($catid);
			
			 
			
			//判断产品是否在购物车
			$userid=M("login")->userid;
			$cartPros=MM("b2b","b2b_cart")->getProductAmount("userid=".$userid." AND ksid=0");
			if($data){
				foreach($data as $k=>$v){
					$v["incart"]=0;
					$v["cart_amount"]=0;
			...
			$this->smarty->display("b2b_product/list.html");
		}
		...
		public function onRecList(){
			...
			$this->smarty->display("b2b_product/reclist.html");
		}
		
		public function onShow(){
			$id=get_post("id","i");
			$data=MM("b2b","b2b_product")->selectRow(array("where"=>"id=".$id));
			...
			$ksList=MM("b2b","b2b_product_ks")->getListByTitle($id);
			if($ksList){
				$ks=$ksList[0];
				$ksid=$ks["id"];
				$ksList2=MM("b2b","b2b_product_ks")->select(array(
			...
			$cart_amount=0;
			$userid=M("login")->userid;
			$cart=MM("b2b","b2b_cart")->selectRow("userid=".$userid." AND productid=".$id." AND ksid=".$ksid);
			if($cart){
				$cart_amount=$cart["amount"];
			}
			$shopid=$data["shopid"];
			$shop=MM("b2b","b2b_shop")->get($shopid,"shopid,shopname,imgurl,raty_grade,buy_num");
			$this->smarty->goassign(array(
				"data"=>$data,
				"ksid"=>$ksid,
				"cart_amount"=>$cart_amount,
			...
			$this->smarty->display("b2b_product/show.html");
		}

<?php
class jdo2o_tixianControl extends skymvc{
	public $sw;
	public $k;
	public function __construct(){
		parent::__construct();	
		$this->k="mod_jdo2o_shop";
	...
	public function onList(){
		$limit=20;
		$where= $this->sw;
		$url="/moduleshop.php?m=jdo2o_tixian";
		...
		$this->smarty->display("jdo2o_tixian/list.html");
	}
	
	
	public function onDefault(){
		$shop=M("mod_jdo2o_shop")->selectRow("shopid=".SHOPID);
		$shopmoney=M("mod_jdo2o_shop_money")->selectRow("shopid=".SHOPID);
		$last=M("tixian")->selectRow(array(
			"order"=>"id DESC",
			"where"=>$this->sw,
			"limit"=>1
		));
		$bankList=M("mod_jdo2o_bankcard")->select(array(
			"where"=>"shopid=".SHOPID
		));	
		$this->smarty->goassign(array(
			"shop"=>$shop,
		...
		$this->smarty->display("jdo2o_tixian/index.html");
	}
	...
	public function onSave(){
		...
		$shop=M("mod_jdo2o_shop_money")->selectRow("shopid=".SHOPID);
		if($shop['balance']<$money){
			$this->goAll("提现余额不足",2);
		}
		$pp=MM("jdo2o","jdo2o_shop_paypwd")->get(SHOPID);
		if(!$pp){
			$this->goAll("请先设置支付密码",1,0,"/moduleshop.php?m=jdo2o_shop_paypwd");
		}
		...
		$bank=M("mod_jdo2o_bankcard")->selectRow("id=".$bankid);

<?php
class pdd_couponModel extends model{
	 
	public function __construct(){
		parent::__construct();
		 
		$this->table="mod_pdd_coupon";
	}
	...
	public function listByShop($userid,$shopid){
		...
		$da=M("mod_pdd_coupon_user")->select(array("where"=>$where));
		if($da){
			foreach($da as $v){
				$ids[]=$v['coupon_id'];
			}
	...
	public function UseList($shopid,$userid,$money){
		...
		$list=M("mod_pdd_coupon")->select(array("where"=>$where));
		//已获取的券
		$where=" status=0 AND shopid=".$shopid."  AND userid=".$userid;
		$da=M("mod_pdd_coupon_user")->select(array("where"=>$where));
		$coupons=array();

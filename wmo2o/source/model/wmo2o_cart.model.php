<?php
	class wmo2o_cartModel extends model{
		public $table="mod_wmo2o_cart";
		...
		public function Dselect($option=array()){
			...
			if($res){
				...
				$pros=MM("wmo2o","wmo2o_product")->getListByIds($productids);
				$kss=MM("wmo2o","wmo2o_product_ks")->getListByIds($ksids);
				foreach($res as $k=> $rs){
		...
		public function getProductAmount($where){
			$list=MM("wmo2o","wmo2o_cart")->select(array("where"=>$where));
			if($list){
				foreach($list as $v){

<?php
class test_b2cControl extends skymvc{
	public $md="b2c";
	...
	public function onGroup(){
		M("mod_b2c_group")->delete("1");
		
		$gid=M("mod_b2c_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"recommend",
			"gnum"=>12,
			"status"=>1
		));
		$gid2=M("mod_b2c_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"hot",
			"gnum"=>12,
			"status"=>1
		));
		$list=M("mod_b2c_product")->selectCols(array(
			"where"=>" status=1 ",
			"limit"=>24
		));

	...
	*/
	...
	class csc_group_productControl extends skymvc{
		...
		public function onDefault(){
			$gid=get("gid","i");
			$group=M("mod_csc_group")->selectRow("gid=".$gid);
			$where=" gid=".$gid;
			$url="/moduleadmin.php?m=csc_group_product&gid=".$gid;
			$limit=24;
			...
			$rscount=true;
			$data=M("mod_csc_group_product")->select($option,$rscount);
			if($data){
				foreach($data as $v){
					$proids[]=$v["productid"];
				}
		...
		public function onDelete(){
			...
			M("mod_csc_group_product")->delete("gpid=$gpid");
			$this->goAll("删除成功");
		...
		public function onOrderIndex(){
			...
			M("mod_csc_group_product")->update(array(

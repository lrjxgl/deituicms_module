	...
	*/
	...
	class csc_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=csc_product&a=default";
			$limit=20;
			...
			$groupList=M("mod_csc_group")->select(array(
				"where"=>" status=1"
			));	
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_csc_product")->selectRow(array("where"=>"id=".$id));
		...
		public function onSave(){
			$id=get_post("id","i");
			$data=M("mod_csc_product")->postData();
		...
		public function onGroup(){
			...
			$hasids=M("mod_csc_group_product")->selectCols(array(

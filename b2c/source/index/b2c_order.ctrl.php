...
class b2c_orderControl extends skymvc{
	...
	public function onOrder(){
		...
		foreach($cartList as $v){
			if($v["otype"]=="isflash"){
				//检测抢购权限
				$que=M("mod_b2c_flash_queue")->selectRow("productid=".$v["productid"]." AND userid=".$userid." AND ksid=".$v["ksid"]." AND status=0 ");
				if(empty($que)){
					$this->goAll("暂无抢购资格",1);
				}
				if($v["stime"]>time()){
		...
		foreach($cartList as $v){
			...
			if($v["otype"]=="isflash"){
				//检测抢购权限
				M("mod_b2c_flash_queue")->update(array(
					"status"=>1
				),"productid=".$v["productid"]." AND userid=".$userid." AND ksid=".$v["ksid"]." AND status=0 ");

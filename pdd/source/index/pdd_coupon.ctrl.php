	...
	*/
	...
	class pdd_couponControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/module.php?m=pdd_coupon&a=default";
			$limit=24;
		...
		public function onShow(){
			...
			$this->smarty->display("pdd_coupon/show.html");
		}
		
		public function onGetCoupon(){
			M("login")->checklogin(1);
			$id=get('id','i');
			$coupon=M("mod_pdd_coupon")->selectRow("id=".$id);
			if($coupon['status']!=1){
				$this->goAll("优惠券不可用",1);
			...
			$ct=M("mod_pdd_coupon_user")->SelectOne(array(

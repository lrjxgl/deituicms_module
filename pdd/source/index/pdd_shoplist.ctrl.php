<?php
class pdd_shoplistControl extends skymvc{
	...
	public function onDefault(){
		...
		if($catid){
			...
			$cat=MM("pdd","pdd_shop_category")->selectRow("catid=".$catid);
		}
		$order=" shopid DESC ";
		$fields=" * ";
		$gps=gps_get();
		...
		$orderList=MM("pdd","pdd_shop")->orderList();
		$filterList=array(
			"send_free"=>"免费配送",
			"send_all"=>"零元起送",
			"cspf"=>"超时赔付",
		...
		$rscount=true;
		$shopList=MM("pdd","pdd_shop")->DselectWindow($option,$rscount);
		...
		$this->smarty->display("pdd_shoplist/index.html");
	}

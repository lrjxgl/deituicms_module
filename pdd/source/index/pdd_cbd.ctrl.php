...
class pdd_cbdControl extends skymvc{
	...
	public function onDefault(){
		$list=M("mod_pdd_cbd")->select(array(
			"where"=>" status=1 "
		));
		if($list){
			foreach($list as $k=>$v){
	...
	public function onShow(){
		...
		$sql="select s.*,b.cbdid from ".table('mod_pdd_cbd_shop')." as b 
			left join ".table('mod_pdd_shop')." as s
			on b.shopid=s.shopid
			where b.cbdid=".$cbdid."
		" ;
		$data=MM("pdd","pdd_shop")->getAll($sql);
		if($data){
			foreach($data as $v){
				$shopids[]=$v["shopid"];
			}
			$pros=MM("pdd","pdd_product")->select(array(
		...
		$this->smarty->display("pdd_cbd/show.html");
	}
	...
	public function onCbdShop(){
		...
		$sql="select s.*,b.cbdid from ".table('mod_pdd_cbd_shop')." as b 
			...
			where b.cbdid=".$cbdid."
		" ;
		$list=MM("pdd","pdd_shop")->getAll($sql);
		$this->smarty->goAssign(array(
			"list"=>$list,
			"cbd"=>$cbd
		));

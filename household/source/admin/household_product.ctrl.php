	...
	*/
	...
	class household_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=household_product&a=default";
			if($pintuan){
				$where.=" AND pt_open=1 ";
				$url.="&pintuan=1";
			}
			...
			if($data){
				...
				$cats=MM("household","household_category")->getListByIds($cids);
				foreach($data as $k=>$v){
			...
			$groupList=M("mod_household_group")->select(array(
				"where"=>" status=1"
			));	
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_household_product")->selectRow(array("where"=>"id=".$id));
				$bb=M("mod_household_product_data")->selectRow(array(
					"where"=>"id=".$id,
					"fields"=>"content,fysm"
				));
				$data["content"]=$bb["content"];
				$data["fysm"]=$bb["fysm"];
			...
			$this->smarty->display("household_product/add.html");
		}
		...
		public function onSave(){
			...
			$data=M("mod_household_product")->postData();
			$data["stime"]=$stime;
			$data["etime"]=$etime;
			if($data["otype"]=="ispin"){
				$data["pt_open"]=1;
			...
			if($id){
				...
				M("mod_household_product")->update($data,"id='$id'");
				M("mod_household_product_data")->update($sdata,"id='$id'");
			}else{
				$data["ex_table_data_id"]=M("table_data")->saveTable($cat["ex_table_id"],0);
				$id=M("mod_household_product")->insert($data);
				$sdata["id"]=$id;
				M("mod_household_product_data")->insert($sdata);
			}
			$this->goall("保存成功");
		}
		...
		public function onStatus(){
			$id=get_post('id',"i");
			$row=M("mod_household_product")->selectRow("id=".$id);
			$status=1;
			if($row["status"]==1){
				$status=2;
			}
			M("mod_household_product")->update(array(
				"status"=>$status
			),"id=".$id);
		...
		public function onDelete(){
			$_GET["ajax"]=1;
			$id=get_post('id',"i");
			M("mod_household_product")->update(array("status"=>11),"id=$id");
			$this->goAll("删除成功");
		...
		public function onCategory(){
			...
			if($ids){
				foreach($ids as $id){
					M("mod_household_product")->update(array("catid"=>$catid),"id=".$id);
				}
			}
			$this->goall("修改成功");
		}
		public function onGroup(){
			...
			$hasids=M("mod_household_group_product")->selectCols(array(
			...
			if(!empty($newids)){
				foreach($newids as $productid){
					M("mod_household_group_product")->insert(array(
						"gid"=>$gid,
						"productid"=>$productid,
						"orderindex"=>11
					));
		...
		public function onTable(){
			//增加扩展表
			$catid=get("catid","i");
			$cat=M("mod_household_category")->selectRow("catid=".$catid);
			$fieldsList=array();
			if($cat){
				$tableid=$cat["ex_table_id"];
			...
			$this->smarty->display("household_product/tablefields.html");
		}
	}

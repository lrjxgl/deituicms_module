	...
	*/
	...
	class pdd_shopControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=pdd_shop&a=default";
			$limit=20;
			...
			$rscount=true;
			$data=M("mod_pdd_shop")->select($option,$rscount);
		...
		public function onAdd(){
			...
			if($shopid){
				$data=M("mod_pdd_shop")->selectRow(array("where"=>"shopid=".$shopid));	
			}
			...
			$this->smarty->display("pdd_shop/add.html");
		}
		...
		public function onStatus(){
			$shopid=get_post('shopid',"i");
			$row=M("mod_pdd_shop")->selectRow("shopid=".$shopid);
			$status=1;
			if($row["status"]==1){
				$status=2;
			}
			M("mod_pdd_shop")->update(array(
				"status"=>$status
			),"shopid=".$shopid);
		...
		public function onDelete(){
			$shopid=get_post('shopid',"i");
			M("mod_pdd_shop")->update(array("status"=>11),"shopid=$shopid");
			$this->goAll("删除成功");

<?php
class test_flkControl extends skymvc{
	public $md="flk";
	...
	public function onNav(){
		$catids=M("mod_flk_category")->selectCols(array(
			"where"=>" status=1 ",
			"fields"=>" catid"
		));
	...
	public function onSHop(){
		...
		foreach($arr as $a){
			...
			if(!M("mod_flk_shop")->selectRow("shopname='".$barr[0]."'")){
	...
	public function onShop_product_category(){
		$shopList=M("mod_flk_shop")->select(array());
		$cats=array(
			"吃的",
			"穿的",
			"用的",
		...
		M("mod_flk_shop_product_category")->delete("1");
		foreach($shopList as $shop){
			shuffle($cats);
			foreach($cats as $k=>$cat){
				M("mod_flk_shop_product_category")->insert(array(
					"title"=>$cat,
					"shopid"=>$shop["shopid"],
					"status"=>1
				));
	...
	public function onShop_product(){
		...
		$table="mod_flk_product";
		$c=file_get_contents($file);
		$res=json_decode($c,true);
		M($table)->delete("1");
		$mcids=M("mod_flk_category")->selectCols(array(
			"fields"=>"catid",
			"where"=>" status=1 "
		));
		foreach($shopList as $shop){
			$catids=M("mod_flk_shop_product_category")->selectCols(array(
				"fields"=>"catid",

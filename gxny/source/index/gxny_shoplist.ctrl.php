<?php
class gxny_shoplistControl extends skymvc{
	...
	public function onDefault(){
		...
		if($catid){
			...
			$cat=MM("gxny","gxny_shop_category")->selectRow("catid=".$catid);
		}
		$order=" shopid DESC ";
		$fields=" * ";
		$gps=gps_get();
		...
		$orderList=MM("gxny","gxny_shop")->orderList();
		$filterList=array(
			"send_free"=>"免费配送",
			"send_all"=>"零元起送",
			"cspf"=>"超时赔付",
		...
		$rscount=true;
		$shopList=MM("gxny","gxny_shop")->Dselect($option,$rscount);
		...
		$this->smarty->display("gxny_shoplist/index.html");
	}

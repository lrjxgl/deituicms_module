	...
	*/
	...
	class wmo2o_shopControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=wmo2o_shop&a=default";
			$limit=20;
			...
			$rscount=true;
			$data=M("mod_wmo2o_shop")->select($option,$rscount);
			if($data){
		...
		public function onAdd(){
			...
			if($shopid){
				$data=M("mod_wmo2o_shop")->selectRow(array("where"=>"shopid=".$shopid));	
			}
			...
			$this->smarty->display("wmo2o_shop/add.html");
		}
		...
		public function onStatus(){
			$shopid=get_post('shopid',"i");
			$row=M("mod_wmo2o_shop")->selectRow("shopid=".$shopid);
			$status=1;
			if($row["status"]==1){
				$status=2;
			}
		...
		public function onDelete(){
			$shopid=get_post('shopid',"i");
			M("mod_wmo2o_shop")->update(array("status"=>11),"shopid=$shopid");
			$this->goAll("删除成功");

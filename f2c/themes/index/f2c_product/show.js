...
$(document).on("click", "#ks1 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=f2c_product_ks&a=sizeList&ajax=1&id=" + kid,
		dataType: "json",
...
$(document).on("click", "#ks2 .kslist-item", function() {
	...
	var that = $(this);
	$.ajax({
		url: "/module.php?m=f2c_product_ks&a=get&ajax=1&id=" + kid,
		dataType: "json",
		success: function(res) {
			that.addClass("kslist-active").siblings().removeClass("kslist-active");
			ksid = res.data.ksid;
		...
		success: function(res) {
			skyToast(res.message);
			 ...
			 if(goBuy){
				 ...
				 window.location="/module.php?m=f2c_order&a=confirm&cartid="+res.data.cartid
			 }
		}
	})
})

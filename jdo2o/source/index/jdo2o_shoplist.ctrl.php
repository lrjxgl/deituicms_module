<?php
class jdo2o_shoplistControl extends skymvc{
	...
	public function onDefault(){
		...
		if($catid){
			...
			$cat=MM("jdo2o","jdo2o_shop_category")->selectRow("catid=".$catid);
		}
		$order=" shopid DESC ";
		$fields=" * ";
		$gps=gps_get();
		...
		$orderList=MM("jdo2o","jdo2o_shop")->orderList();
		$filterList=array(
			"send_free"=>"免费配送",
			"send_all"=>"零元起送",
			"cspf"=>"超时赔付",
		...
		$rscount=true;
		$shopList=MM("jdo2o","jdo2o_shop")->Dselect($option,$rscount);
		...
		$shop=MM("jdo2o","jdo2o_shop")->get(SHOPID,"shopid,shopname,imgurl,address,yystatus");
		if($shop["yystatus"]==1){
		...
		$this->smarty->display("jdo2o_shoplist/index.html");
	}

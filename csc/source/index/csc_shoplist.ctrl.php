<?php
class csc_shoplistControl extends skymvc{
	...
	public function onDefault(){
		...
		if($catid){
			...
			$cat=MM("csc","csc_shop_category")->selectRow("catid=".$catid);
		}
		$order=" shopid DESC ";
		$fields=" * ";
		$gps=gps_get();
		...
		$orderList=MM("csc","csc_shop")->orderList();
		$filterList=array(
			"send_free"=>"免费配送",
			"send_all"=>"零元起送",
			"cspf"=>"超时赔付",
		...
		$rscount=true;
		$shopList=MM("csc","csc_shop")->Dselect($option,$rscount);
		...
		$shop=MM("csc","csc_shop")->get(SHOPID,"shopid,shopname,imgurl,address,send_area,yystatus");
		if($shop["yystatus"]==1){
		...
		$this->smarty->display("csc_shoplist/index.html");
	}

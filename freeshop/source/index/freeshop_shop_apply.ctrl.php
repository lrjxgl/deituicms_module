...
class freeshop_shop_applyControl extends skymvc{
	...
	public function onDefault(){
		...
		$apply=M("mod_freeshop_shop_apply")->selectRow("userid=".$userid);
		$shop=M("mod_freeshop_shop")->selectRow("userid=".$userid);
		if($shop){
			$this->goAll("你已经成功入驻了",1,0,"/module.php?m=freeshop");
		}
	...
	public function onSendSms(){
		...
		$exkey="freeshopshopapplysms".$telephone;
		$key="freeshopshopapplysms".$telephone.$yzm;
		$t=cache()->get($exkey);
		if($t){
	...
	public function onSave(){
		...
		$data=M("mod_freeshop_shop_apply")->postData();
		...
		$data["userid"]=$userid;
		M("mod_freeshop_shop_apply")->insert($data);

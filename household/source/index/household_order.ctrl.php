...
class household_orderControl extends skymvc{
	...
	public function onConfirm(){
		...
		$cartList=MM("household","household_cart")->Dselect(array(
			"where"=>$where
		));
		if(empty($cartList)){
		...
		*/
		...
		$this->smarty->display("household_order/confirm.html");
	}
	public function onBuy(){
		
		$this->smarty->display("household_order/buy.html");
	}
	public function onOrder(){
		...
		*/
		...
		$orderid=MM("household","household_order")->insert(array(
			"money"=>$total_money,
			"paymoney"=>$paymoney,
			"ispay"=>$ispay,
			"vipmoney"=>$vipmoney,
		...
		foreach($cartList as $v){
			...
			$prolist[]=$v;
			M("mod_household_order_product")->insert(array(
				"orderid"=>$orderid,
				"createtime"=>$createtime,
				"userid"=>$userid,
		...
		//生成订单地址
		M("mod_household_order_address")->insert(array(
			"orderid"=>$orderid,
			"createtime"=>$createtime,
			"userid"=>$userid,
		...
		M("mod_household_order_data")->insert(array(		 
			"orderid"=>$orderid,
			"userid"=>$userid,
			"createtime"=>date("Y-m-d H:i:s"),
			"updatetime"=>date("Y-m-d H:i:s"),
		...
		*/	
		//清除购物车
		MM("household","household_cart")->delete(" id in("._implode($cartids).")");
		...
		if(!$ispay){
			...
			if($ispin){
				MM("household","household_order")->pinOrder($orderid);
			}
			$rdata=array(
				"action"=>"success",
				"orderid"=>$orderid
		...
		//通知网站
		M("site_msg")->add([
			"tablename"=>"household_order",
			"content"=>"有人在家政派单中心下单了"
		]);
	...
	public function onSuccess(){
		...
		$this->smarty->display("household_order/success.html");
	}
	...
	public function onMy(){
		$userid=M("login")->userid;
		$where=" userid=".$userid;
		$url="/module.php?m=household_order&a=my";
		$limit=12;
		...
		$rscount=true;
		$data=MM("household","household_order")->select($option,$rscount);
		if($data){
			foreach($data as $v){
				$oids[]=$v["orderid"];
			}
		...
		$this->smarty->display("household_order/my.html");
	}
	...
	public function onReceive(){
		...
		MM("household","household_order")->begin();
		MM("household","household_order")->finish($order,"user");
		//给师傅发布推送
		MM("household","household_sender_notice")->add(array(
			"content"=>"您的家政订单,用户完成验单了",
	...
	public function onRaty(){
		...
		$sql="select a.id,a.raty_grade,a.productid,b.title,b.imgurl from ".table("mod_household_order_product")." as a "
			." left join ".table("mod_household_product")." as b "
			." on a.productid=b.id "
			." where a.orderid=".$orderid;
		$proList=MM("household","household_order")->getAll($sql);
		if($proList){
		...
		$raty=MM("household","mod_household_order_raty")->selectRow("orderid=".$orderid);
		if(empty($raty)){
			$raty=array(
				"raty_quality"=>9,
				"raty_service"=>9,
		...
		$this->smarty->display("household_order/raty.html");
	}
	...
	public function onRatySave(){
		...
		if(!$order["israty"]){
			...
			$ratyData=M("mod_household_order_raty")->postData();
			$ratyData["userid"]=$userid;
			$ratyData["createtime"]=date("Y-m-d H:i:s");
			M("mod_household_order_raty")->insert($ratyData);
			$ratyPros=post("ratyPros","i");
			if(!empty($ratyPros)){
				foreach($ratyPros as $k=>$v){
					M("mod_household_order_product")->update(array(
						"raty_grade"=>intval($v),
						"iscomment"=>1,
						"content"=>$content

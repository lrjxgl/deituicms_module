	...
	*/
	...
	class wmo2o_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=wmo2o_product&a=default";
			$limit=20;
			...
			if($data){
				...
				$cats=MM("wmo2o","wmo2o_category")->getListByIds($cids);
				foreach($data as $k=>$v){
			...
			$groupList=M("mod_wmo2o_group")->select(array(
				"where"=>" status=1"
			));	
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_wmo2o_product")->selectRow(array("where"=>"id=".$id));
		...
		public function onSave(){
			$id=get_post("id","i");
			$data=M("mod_wmo2o_product")->postData();
		...
		public function onGroup(){
			...
			$hasids=M("mod_wmo2o_group_product")->selectCols(array(

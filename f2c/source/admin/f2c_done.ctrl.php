...
class f2c_doneControl extends skymvc{
	...
	public function onDefault(){
		$url="/moduleadmin.php?m=f2c_done";
		...
		if($team_nickname){
			$teamids=MM("f2c","f2c_team")->selectCols(array(
		...
		if($shequ_title){
			$scids=MM("f2c","f2c_shequ")->selectCols(array(
		...
		$rscount=true;
		$list=MM("f2c","f2c_done")->Dselect($option,$rscount);
	...
	public function onLast(){
		$url="/moduleadmin.php?m=f2c_done&a=last";
		$month=date("m");
		if($month>1){
			$sm=date("Ym")-1;
		}else{
		...
		$this->smarty->display("f2c_done/last.html");
	}
	
	public function onAll(){
		$url="/moduleadmin.php?m=f2c_done&a=all";
		$start=get("per_page","i");
		$limit=100;
		$where=" 1 ";
		$month=get("month","i");
		...
		$this->smarty->display("f2c_done/all.html");
	}
}
	...
	*/
	...
	class fishing_free_placeControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=fishing_free_place&a=default";
			$limit=20;
			...
			$rscount=true;
			$data=M("mod_fishing_free_place")->select($option,$rscount);
		...
		public function onStatus(){
			$placeid=get_post('placeid',"i");
			$row=M("mod_fishing_free_place")->selectRow("placeid=".$placeid);
			if($row["status"]==1){
				$status=2;
			}else{
				$status=1;
			...
			M("mod_fishing_free_place")->update(array("status"=>$status),"placeid=".$placeid);
			$this->goall("状态修改成功",0,$status);
		}
		
		public function onDelete(){
			$placeid=get_post('placeid',"i");
			M("mod_fishing_free_place")->update(array("status"=>11),"placeid=".$placeid);
			$this->goAll("删除成功");

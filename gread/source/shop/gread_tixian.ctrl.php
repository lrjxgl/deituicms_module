<?php
class gread_tixianControl extends skymvc{
	public $sw;
	public $k;
	public function __construct(){
		parent::__construct();	
		$this->k="mod_gread_shop";
	...
	public function onList(){
		$limit=20;
		$where= $this->sw;
		$url="/moduleshop.php?m=gread_tixian";
		...
		$this->smarty->display("gread_tixian/list.html");
	}
	
	
	public function onDefault(){
		$shop=M("mod_gread_shop")->selectRow("shopid=".SHOPID);
		$shopmoney=M("mod_gread_shop_money")->selectRow("shopid=".SHOPID);
		$last=M("tixian")->selectRow(array(
			"order"=>"id DESC",
			"where"=>$this->sw,
			"limit"=>1
		));
		$bankList=M("mod_gread_bankcard")->select(array(
			"where"=>"shopid=".SHOPID
		));	
		$this->smarty->goassign(array(
			"shop"=>$shop,
		...
		$this->smarty->display("gread_tixian/index.html");
	}
	...
	public function onSave(){
		...
		$shop=M("mod_gread_shop_money")->selectRow("shopid=".SHOPID);
		...
		$bank=M("mod_gread_bankcard")->selectRow("id=".$bankid);

...
var app = new Vue({
	...
	data: {
		...
		pageLoad:false,
		show:"flex",
		catActive:"catNav-swiper-slide-active",
		ksShow:false,
		ksproduct:[],
		ksList:[],
		ksList2:[],
	...
	methods: {
		goProduct:function(id){
			window.location="/module.php?m=s2c_product&a=show&id="+id;
		},
		getPage: function() {
			var that = this;
			$.get("/module.php?m=s2c_product&ajax=1", function(res) {
				that.pageData = res.data;
				that.pageLoad=true;
				
				setTimeout(function(){
					that.catNavY=$("#catNav").offset().top
					$(window).on("scroll",function(e){
						var st=$(window).scrollTop();
						 
						if(st>that.catNavY-50){
							$("#catNav").addClass("catNav-fixed");
							$(".catNav-fixed-show").show();
						}else{
							$("#catNav").removeClass("catNav-fixed");
							$(".catNav-fixed-show").hide();
						}
					}) 
					
					var navFlash = new Swiper("#catNav", {
						slidesPerView: 4,
						freeMode: true
					})
		...
		getList:function(){
			var that=this;
			$.ajax({
				url:"/module.php?m=s2c_product&ajax=1",
				data:{
					catid:catid
				},
				dataType:"json",
		...
		setCat:function(cid){
			...
			catid=cid;
			if(catid==0){
				that.catActive="catNav-swiper-slide-active";
			}else{
				that.catActive='';
			}
			this.getList();
		...
		addCart: function(id, ksid) {
			...
			var amount = 1;
			$.ajax({
				url: '/module.php?m=s2c_cart&a=add&ajax=1',
				data: {
					productid: id,
					amount: amount,
					ksid: ksid
		...
		ksBox:function(id){
			var that=this;
			$.ajax({
				url:"/module.php?m=s2c_product_ks&ajax=1&productid="+id,
				dataType:"json",
				success:function(res){
					that.ksShow=true;
		...
		ks1:function(id){
			var that=this;
			$.ajax({
				url:"/module.php?m=s2c_product_ks&a=sizeList&ajax=1&id="+id,
				dataType:"json",
				success:function(res){
					that.ksid1=res.data.ksid;
					that.ksid=res.data.ksid;
		...
		ks2:function(id){
			...
			that.ksid=id;
			 $.ajax({
			 	url:"/module.php?m=s2c_product_ks&a=get&ajax=1&id="+id,
			 	dataType:"json",
			 	success:function(res){

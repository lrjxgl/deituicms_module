<?php
class csc_openControl extends skymvc{
	...
	public function onDefault(){
		$wexin=M("mod_csc_shop_openbind")->selectRow("shopid=".SHOPID." AND k='weixin'");
		$this->smarty->goAssign(array(
			"weixin"=>$weixin
		));
		$this->smarty->display("csc_open/index.html");
	}
	public function onGoWeixin(){
		$wx=M("weixin")->selectRow(array("where"=>"","order"=>"id DESC"));
		$redirect_uri=HTTP_HOST."/moduleshop.php?m=csc_open&a=BindWeixin&wid=".$wx['id'];
	...
	public function onBindWeixin(){
		...
		if(isset($data['access_token']) && !empty($data['openid'])){
			$row=M("mod_csc_shop_openbind")->selectRow("shopid=".SHOPID." AND k='weixin'");
			$content=arr2str(array(
				"openid"=>$data["openid"],
				"access_token"=>$data["access_token"]
			));
			if($row){
				M("mod_csc_shop_openbind")->update(array(
					"content"=>$content
				),"id=".$row["id"]);
			}else{
				M("mod_csc_shop_openbind")->insert(array(
					"shopid"=>SHOPID,
					"k"=>"weixin",
					"content"=>$content
				));

<?php
class wmo2o_orderControl extends skymvc{
	...
	public function onConfirm(){
		...
		$shopid=get("shopid","i");
		$shop=MM("wmo2o","wmo2o_shop")->get($shopid,"shopid,shopname,imgurl,yystatus");
		if(empty($shop)){
			$this->goAll("请选择商家",1);
		}
		if($shop["yystatus"]!=1){
		...
		//优惠券
		$coupon_id=0;
		$couponList=MM("wmo2o","wmo2o_coupon")->UseList($shopid,$userid,$total_money);
		if($couponList){
		...
		$this->smarty->display("wmo2o_order/confirm.html");
	}
	public function onBuy(){
		
		$this->smarty->display("wmo2o_order/buy.html");
	}
	public function onOrder(){
		...
		if($coupon_id){
			...
			$coupon=M("mod_wmo2o_coupon")->selectRow("id=".$coupon_id);
			$etime=strtotime($coupon['end_time']);
			if(!$coupon || $goods_money<$coupon['lower_money'] || $etime<time()){
				//失效
			}else{
				$coupon_user=M("mod_wmo2o_coupon_user")->selectRow("coupon_id=".$coupon_id." AND status=0 AND userid=".$userid);			
				if($coupon_user || $coupon["typeid"]==1){
					$coupon_money=$coupon['money'];
					if($coupon_user){
						M("mod_wmo2o_coupon_user")->update(array("status"=>1),"id=".$coupon_user['id']);
					}				
					M("mod_wmo2o_coupon")->update(array("use_num"=>$coupon['use_num']+1),"id=".$coupon_id);
		...
		MM("wmo2o","wmo2o_order")->begin();
		$orderid=MM("wmo2o","wmo2o_order")->insert(array(
			"shopid"=>$shopid,
			"money"=>$total_money,
			"goods_money"=>$goods_money,
			"express_money"=>$express_money,
		...
		foreach($cartList as $v){
			$prolist[]=$v;
			M("mod_wmo2o_order_product")->insert(array(
				"shopid"=>$shopid,
				"orderid"=>$orderid,
				"createtime"=>$createtime,
				"userid"=>$userid,
		...
		//生成订单地址
		M("mod_wmo2o_order_address")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"createtime"=>$createtime,
			"userid"=>$userid,
		...
		M("mod_wmo2o_order_data")->insert(array(
			"shopid"=>$shopid,
			"orderid"=>$orderid,
			"userid"=>$userid,
			"createtime"=>date("Y-m-d H:i:s"),
		...
		foreach($cartList as $v){
			if($v["ksid"]){
				MM("wmo2o","wmo2o_product_ks")->changenum("total_num",-$v["amount"],"id=".$v["ksid"]);
			}else{
				MM("wmo2o","wmo2o_product")->changenum("total_num",-$v["amount"],"id=".$v["productid"]);
			}
		}	
		//清除购物车
		MM("wmo2o","wmo2o_cart")->delete(" id in("._implode($cartids).")");
		MM("wmo2o","wmo2o_order")->commit();
		...
		if(!$data['ispay']){
			...
			/**发送通知**/
			MM("wmo2o","wmo2o_shop_notice")->sendNewOrder($orderid);
			$rdata["action"]="finish";
		}
	...
	public function onPay($return=0){
		...
		$order=MM("wmo2o","wmo2o_order")->selectRow("orderid=".$orderid);
		//生成支付
		...
		$orderdata=array(
			"table"=>"plugin",
			"callback"=>'
				MM("wmo2o","wmo2o_order")->update(array(
					"ispay"=>1,
	...
	public function onSuccess(){
		$this->smarty->display("wmo2o_order/success.html");
	}
	public function onMy(){
		$userid=M("login")->userid;
		$where=" userid=".$userid;
		$url="/module.php?m=wmo2o_order&a=my";
		$limit=24;
		...
		$rscount=true;
		$data=MM("wmo2o","wmo2o_order")->select($option,$rscount);
		if($data){
			foreach($data as $v){
				$oids[]=$v["orderid"];
				$shopids[]=$v["shopid"];
			}
			$sps=MM("wmo2o","wmo2o_shop")->getListByIds($shopids);
			$ods=MM("wmo2o","wmo2o_order_data")->getListByOrderIds($oids);
			foreach($data as $k=>$v){
				$v["shop"]=$sps[$v["shopid"]];
				$v['addr']=$ods[$v['orderid']]['address'];
				$v['prolist']=$ods[$v['orderid']]['prolist'];
				$v["status_name"]=MM("wmo2o","wmo2o_order")->getStatus($v);
		...
		$this->smarty->display("wmo2o_order/my.html");
	}
	public function onShow(){
		$orderid=get("orderid","i");
		$order=MM("wmo2o","wmo2o_order")->selectRow("orderid=".$orderid);
		$orderdata=MM("wmo2o","wmo2o_order_data")->get($orderid);
		$shop=MM("wmo2o","wmo2o_shop")->get($order["shopid"]); 
		...
		if($order["ispay"] && $order["status"]<3){
			$ordercode=MM("wmo2o","wmo2o_order_code")->get($orderid,$order["shopid"]);
			$c=json_encode(array(
				"action"=>"url",
				"url"=>"../wmo2o_order_code/index?ordercode=".$ordercode
			));
		...
		$this->smarty->display("wmo2o_order/show.html");
	}
	/**取消**/
	public function onCancel(){
		...
		if($order["ispay"]==1){
			...
			M("refund_apply")->insert(array(
				...
				"money"=>$order['money'],
				"recharge_id"=>$order['recharge_id'],
				"content"=>"wmo2o订单取消，申请退回支付渠道",
				"odata"=>base64_encode(json_encode($odata))
			));
		}
		//增加商品库存
		$proList=MM("wmo2o","wmo2o_order_product")->select(array(
			"where"=>"orderid=".$orderid
		));
		if($proList){
			foreach($proList as $v){
				if($v["ksid"]){
					MM("wmo2o","wmo2o_product_ks")->changenum("total_num",$v["amount"],"ksid=".$v["ksid"]);
				}else{
					MM("wmo2o","wmo2o_product")->changenum("total_num",$v["amount"],"id=".$v["productid"]);
				}	
			}
	...
	public function onRaty(){
		...
		$sql="select a.id,a.raty_grade,a.productid,b.title,b.imgurl from ".table("mod_wmo2o_order_product")." as a "
			." left join ".table("mod_wmo2o_product")." as b "
			." on a.productid=b.id "
			." where a.orderid=".$orderid;
		$proList=MM("wmo2o","wmo2o_order")->getAll($sql);
		if($proList){
		...
		$raty=MM("wmo2o","mod_wmo2o_order_raty")->selectRow("orderid=".$orderid);
		if(empty($raty)){
			$raty=array(
				"raty_quality"=>9,
				"raty_service"=>9,
		...
		$this->smarty->display("wmo2o_order/raty.html");
	}

	...
	*/
	...
	class house_agentControl extends skymvc{
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=M("mod_house_agent")->select($option,$rscount);
		...
		public function onAdd(){
			...
			$this->smarty->display("house_agent/add.html");
		}
		...
		public function onSave(){
			...
			if($id){
				M("mod_house_agent")->update($data,"id='$id'");
			}else{
		...
		public function onStatus(){
			...
			M("mod_house_agent")->update(array("status"=>$status),"id=$id");
			$this->goall("状态修改成功");
		}
		
		public function onDelete(){
			$id=get_post('id',"i");
			M("mod_house_agent")->update(array("status"=>11),"id=$id");
			$this->goAll("删除成功");

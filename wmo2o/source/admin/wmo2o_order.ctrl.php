	...
	*/
	...
	class wmo2o_orderControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=wmo2o_order&a=default";
			$limit=24;
			...
			$rscount=true;
			$data=M("mod_wmo2o_order")->select($option,$rscount);
			if($data){
				foreach($data as $v){
					$oids[]=$v["orderid"];
					$uids[]=$v["userid"];
			...
			$this->smarty->display("wmo2o_order/index.html");
		}
		...
		public function onConfirm(){
			...
			MM("wmo2o","wmo2o_order_log")->insert(array(
				"adminid"=>$adminid,
				"orderid"=>$orderid,
		...
		public function onAutoFinish(){
			$time=date("Y-m-d H:i:s",time()-3600*24*14);
			$res=MM("wmo2o","wmo2o_order")->select(array(
				"where"=>" status in(1,2) AND ispay=1 AND createtime<'".$time."' ",
			...
			if(!empty($res)){
				...
				foreach($res as $rs){
					MM("wmo2o","wmo2o_order")->begin();
					MM("wmo2o","wmo2o_order")->finish($rs["orderid"],$rs);
					MM("wmo2o","wmo2o_order")->commit();
				}
			}

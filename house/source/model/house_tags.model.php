<?php
class house_tagsModel extends model{
	public $table="mod_house_tags";
	...
	public function Done($objectid,$tags){
		...
		$indexlist=M("mod_house_tags_index")->selectCols(array(
		...
		if($deleteIndex){
			...
			M("mod_house_tags_index")->delete("objectid=".$objectid." AND tagid in("._implode($delTagsId).") ");
		...
		if(!empty($insertIndex)){
			M("mod_house_tags_index")->insertMore($insertIndex);
		}
		 
	}
	
	public function gethouseByKey($gkey){
		$group=$this->selectRow(array(
			"where"=>" gkey='".sql($gkey)."' ",
			 "fields"=>"tagid,gnum"
		));
		if(!$group) return false;
		$data=M("mod_house_tags_index")->select(array(

<?php
	...
	class jdo2o_couponControl extends skymvc{
		...
		public function onDefault(){
			$where=" shopid=".SHOPID;
			$url="/moduleshop.php?m=jdo2o_coupon&a=default";
			$limit=20;
		...
		public function onAdd(){
			...
			$this->smarty->display("jdo2o_coupon/add.html");
		}
		
		public function onSave(){
			
			$id=get_post("id","i");
			$data=M("mod_jdo2o_coupon")->postData();
			$data['shopid']=SHOPID;

			if($id){
				$row=M("mod_jdo2o_coupon")->selectRow(array("where"=>"id={$id}"));
				if($row['shopid']!=SHOPID ){
					$this->goAll("暂无权限");
				}
				M("mod_jdo2o_coupon")->update($data,"id='$id'");
			}else{
				 
				$data["dateline"]=time();
				M("mod_jdo2o_coupon")->insert($data);
			}
		...
		public function onStatus(){
			...
			M("mod_jdo2o_coupon")->update(array("status"=>$status),"id=$id");
			exit(json_encode(array("error"=>0,"message"=>"状态修改成功")));
		}
		
		public function onDelete(){
			$id=get_post('id',"i");
			M("mod_jdo2o_coupon")->update(array("status"=>99),"id={$id}");
		...
		public function onUser(){
			...
			$limit=20;
			$where=" 1=1 ";
			$url="/moduleshop.php?m=jdo2o_coupon&a=user";
			$option=array(
				"where"=>$where,
				"start"=>$start,
				"limit"=>$limit
			...
			$rscount=true;
			$data=M("mod_jdo2o_coupon_user")->select($option,$rscount);
			if($data){
				foreach($data as $k=>$v){
					$uids[]=$v['userid'];
					$cids[]=$v['coupon_id'];
				...
				$cids && $cos=M("mod_jdo2o_coupon")->getByIds($cids);
				foreach($data as $k=>$v){
		...
		public function onuserDelete(){
			$id=get_post('id',"i");
			M("mod_jdo2o_coupon_user")->delete("id={$id}");
			exit(json_encode(array("error"=>0,"message"=>"删除成功")));
		}

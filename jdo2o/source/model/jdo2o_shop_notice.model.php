<?php
class jdo2o_shop_noticeModel extends model{
	public $queue=false;
	public $table="mod_jdo2o_shop_notice";
	public function sendNewOrder($orderid){
		require_once "extends/pushtpl.class.php";
		$open=M("mod_jdo2o_shop_openbind")->selectRow("shopid=".SHOPID." AND k='weixin' ");
		if(empty($open)){
			return false;
		}
		$order=M("mod_jdo2o_order")->selectRow("orderid=".$orderid);
		...
		if($token=get_weixin_access_token()){
			$ops=array(
				"touser"=>$od["openid"],
				"template_id"=>$tpl["key"],
				"url"=>HTTP_HOST."/moduleshop.php?m=jdo2o_order&a=show&orderid=".$orderid,
				"data"=>array(
	...
	public function sendUpdateOrder($orderid,$type="shop"){
		...
		$url=HTTP_HOST."/module.php?m=jdo2o_order&a=show&orderid=".$orderid;
		if($order["status"]==1){
			$content="商家确认订单了";

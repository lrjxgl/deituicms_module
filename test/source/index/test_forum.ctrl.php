<?php
class test_forumControl extends skymvc{
	public $md="forum";
	...
	public function onCategory(){
		...
		M("mod_{$this->md}_group")->query("truncate ".table("mod_{$this->md}_group")." ");
		M("mod_{$this->md}_group")->query("truncate ".table("mod_{$this->md}_category")." ");
		foreach($catlist as $v){
			$row=M("mod_{$this->md}_group")->selectRow("title='".$v["title"]."'");
			$pid=$row["gid"];
			if(empty($row)){
				$pid=M("mod_{$this->md}_group")->insert(array(
					"status"=>1,
					"title"=>$v["title"]
				));
			}
	...
	public function onNav(){
		$catids=M("mod_{$this->md}_group")->selectCols(array(
			"where"=>" status=1 ",
			"fields"=>" gid "
		));
		...
		for($i=1;$i<=8;$i++){
			M("ad")->insert(array(
				...
				"status"=>2,
				"imgurl"=>"attach/demo/nav/{$i}.png",
				"link1"=>"/module.php?m={$this->md}&a=list&gid=".$catids[$i]

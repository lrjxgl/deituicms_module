...
var app = new Vue({
	...
	methods: {
		goProduct:function(shopid,id){
			window.location="/module.php?m=mmjz_product&a=show&shopid="+shopid+"&id="+id;
		},
		getPage: function() {
			var that = this;
			$.get("/module.php?m=mmjz_prolist&ajax=1", function(res) {
		...
		getList:function(){
			...
			$.ajax({
				url:"/module.php?m=mmjz_prolist&ajax=1",
				data:{
					catid:catid,
					per_page:per_page
				},
		...
		addCart: function(shopid,id, ksid) {
			...
			var amount = 1;
			$.ajax({
				url: '/module.php?m=mmjz_cart&a=add&ajax=1',
				data: {
					shopid:shopid,
					productid: id,
					amount: amount,
		...
		ksBox:function(id){
			var that=this;
			$.ajax({
				url:"/module.php?m=mmjz_product_ks&ajax=1&productid="+id,
				dataType:"json",
				success:function(res){
					that.ksShow=true;
		...
		ks1:function(id){
			var that=this;
			$.ajax({
				url:"/module.php?m=mmjz_product_ks&a=sizeList&ajax=1&id="+id,
				dataType:"json",
				success:function(res){
					that.ksid1=res.data.ksid;
					that.ksid=res.data.ksid;
		...
		ks2:function(id){
			...
			that.ksid=id;
			 $.ajax({
			 	url:"/module.php?m=mmjz_product_ks&a=get&ajax=1&id="+id,
			 	dataType:"json",
			 	success:function(res){

	...
	*/
	...
	class fishing_blogControl extends skymvc{
		...
		public function onDefault(){
			...
			$type=get("type","h");
			$url="/moduleadmin.php?m=fishing_blog&type=".$type;
			if($type=="check"){
				$where=" status=0 ";
			}
			$limit=20;
			...
			if($type=="check"){
				$this->smarty->display("fishing_blog/check.html");
			}else{
		...
		public function onSave(){
			...
			if($id){
				M("mod_fishing_blog")->update($data,"id='$id'");
			}else{
		...
		public function onStatus(){
			...
			M("mod_fishing_blog")->update(array("status"=>$status),"id=$id");
			$this->goall("状态修改成功");
		}
		...
		public function onPass(){
			$id=get_post('id',"i");
			M("mod_fishing_blog")->update(array("status"=>1),"id=".$id);
			$this->goAll("审核通过");
		}
		
		public function onForbid(){
			$id=get_post('id',"i");
			M("mod_fishing_blog")->update(array("status"=>2),"id=".$id);
			$this->goAll("审核不通过");
		}

<?php
class car_zucheModel extends model{
	public $table="mod_car_zuche";
	...
	public function parseContent($content){
		...
		if(isset($tps[1])){
			...
			foreach($tps[1] as $tp){
				$content=str_replace("#{$tp}#",'<a class="blogLink" href="/module.php?m=car_zuche&a=topic&title='.urlencode(html($tp)).'">#'.html($tp).'#</a>',$content);
			}
		}
		return $content;
	}

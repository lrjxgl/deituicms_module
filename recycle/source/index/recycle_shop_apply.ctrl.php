...
class recycle_shop_applyControl extends skymvc{
	...
	public function onDefault(){
		...
		$apply=M("mod_recycle_shop_apply")->selectRow("userid=".$userid."   ");
		$shop=M("mod_recycle_shop")->selectRow("userid=".$userid);
		if($shop){
			$this->goAll("你已经成功入驻了",1,0,"/module.php?m=recycle");
		}
	...
	public function onSendSms(){
		...
		$yzm=rand(1111,9999);
		$exkey="recycleshopapplysms".$telephone;
		$key="recycleshopapplysms".$telephone.$yzm;
		$t=cache()->get($exkey);
		if($t){
	...
	public function onSave(){
		...
		$data=M("mod_recycle_shop_apply")->postData();
		...
		$data["userid"]=$userid;
		M("mod_recycle_shop_apply")->insert($data);

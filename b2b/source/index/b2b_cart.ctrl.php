<?php
	class b2b_cartControl extends skymvc{
		...
		public function onAdd(){
			...
			$ksid=get("ksid","i");
			$product=MM("b2b","b2b_product")->selectRow("id=".$productid);
			...
			$amount=get("amount","i");
			$cart=MM("b2b","b2b_cart")->selectRow("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
			$ks=MM("b2b","b2b_product_ks")->selectRow("id=".$ksid);
			if($ks){
			...
			$cart=MM("b2b","b2b_cart")->selectRow("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
			if($amount<=0){
				MM("b2b","b2b_cart")->delete("userid=".$userid." AND productid=".$productid." AND ksid=".$ksid);
				$rdata=array(
					"action"=>"delete",
					"ksid"=>$ksid,
					"productid"=>$productid,
				...
				$cart["id"]=MM("b2b","b2b_cart")->insert(array(	
					"createtime"=>date("Y-m-d H:i:s"),
					"amount"=>$amount,
					"ksid"=>$ksid,
					"productid"=>$productid,
		...
		public function onClear(){
			$userid=M("login")->userid;
			MM("b2b","b2b_cart")->delete("userid=".$userid);
			$this->goAll("清除成功");
		}

var App=new Vue({
	...
	methods:{
		getPage:function(){
			var that=this;
			$.ajax({
				url:"/moduleshop.php?m=csc_product_ks&a=list&ajax=1&productid="+productid,
				dataType:"json",
...
$(document).on("click",".ks-add",function(){
	...
	$.post("/moduleadmin.php?m=csc_product_ks&a=save&ajax=1",{
		productid:productid,
...
$(document).on("click",".ks-del",function(){
	if(confirm('确认删除吗？')){
		var $obj=$(this).parents(".ksitem");
		$.get("/moduleadmin.php?m=csc_product_ks&a=delete&ajax=1&id="+$obj.find(".ks-id").val(),function(data){
			App.getPage();
		})
	}
});
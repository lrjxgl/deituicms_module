	...
	*/
	...
	class s2c_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/module.php?m=s2c_product&type=".$type;
			$limit=20;
			...
			if($data){
				...
				$cats=MM("s2c","s2c_category")->getListByIds($cids);
				foreach($data as $k=>$v){
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_s2c_product")->selectRow(array("where"=>"id=".$id));
			...
			$this->smarty->display("s2c_product/add.html");
		}
		
		public function onSave(){
			$id=get_post("id","i");
			$data=M("mod_s2c_product")->postData();
			$content=post("content","x");
			$sdata=array(
				"content"=>$content
			);
			if($id){
				M("mod_s2c_product")->update($data,"id='$id'");
				M("mod_s2c_product_data")->update($sdata,"id='$id'");
			}else{
				$id=M("mod_s2c_product")->insert($data);
				$sdata["id"]=$id;
				M("mod_s2c_product_data")->insert($sdata);
			}
			$this->goall("保存成功");
		}
		...
		public function onStatus(){
			$id=get_post('id',"i");
			$row=M("mod_s2c_product")->selectRow("id=".$id);
			$status=1;
			if($row["status"]==1){
				$status=2;
			}
			M("mod_s2c_product")->update(array(
				"status"=>$status
			),"id=".$id);
		...
		public function onDelete(){
			$id=get_post('id',"i");
			M("mod_s2c_product")->update(array("status"=>11),"id=$id");
			$this->goAll("删除成功");

<?php
class flk_shopModel extends model{
	public $table="mod_flk_shop";
	...
	public function DselectWindow($option=array(),&$rscount=false){
		...
		if($data){
			...
			//优惠券
			$cps=MM("flk","flk_coupon")->select(array(
	...
	public function favList($userid,$fields="shopid,shopname,imgurl"){
		...
		if(empty($shopids)) return false;
		$sps=$this->getListByIds($shopids,$fields);
		$list=array();

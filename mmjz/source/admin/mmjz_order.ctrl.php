	...
	*/
	...
	class mmjz_orderControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=mmjz_order&a=default";
			$limit=24;
			...
			$rscount=true;
			$data=M("mod_mmjz_order")->select($option,$rscount);
			if($data){
				foreach($data as $v){
					$oids[]=$v["orderid"];
					$uids[]=$v["userid"];
		...
		public function onConfirm(){
			...
			MM("mmjz","mmjz_order_log")->insert(array(
				"adminid"=>$adminid,
				"orderid"=>$orderid,
		...
		public function onAutoFinish(){
			...
			$res=MM("mmjz","mmjz_order")->select(array(
			...
			if(!empty($res)){
				...
				foreach($res as $rs){
					MM("mmjz","mmjz_order")->begin();
					MM("mmjz","mmjz_order")->finish($rs["orderid"],$rs);
					MM("mmjz","mmjz_order")->commit();
				}
			}

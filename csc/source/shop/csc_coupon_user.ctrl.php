<?php
	...
	class csc_coupon_userControl extends skymvc{
		public $shopid;
		public $shop;
		...
		public function onDefault(){
			...
			$url="/moduleshop.php?m=coupon&a=user";
			$option=array(
				"where"=>$where,
				"start"=>$start,
				"limit"=>$limit
			...
			$rscount=true;
			$data=M("mod_csc_coupon_user")->select($option,$rscount);
			if($data){
				foreach($data as $k=>$v){
					$uids[]=$v['userid'];
					$cids[]=$v['coupon_id'];
				...
				$cids && $cos=MM("csc","csc_coupon")->getByIds($cids);
				foreach($data as $k=>$v){
			...
			$this->smarty->display("csc_coupon_user/index.html");
		}
		
		public function onNotice(){
			$id=get_post('id',"i");
			$row=M("mod_csc_coupon_user")->selectRow("id=".$id);
			...
			M("mod_csc_coupon_user")->update(array("notice_time"=>time()),"id=".$id);
			$this->goALL("通知成功",0);
		}

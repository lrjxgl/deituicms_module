<?php
class test_f2cControl extends skymvc{
	public $md="f2c";
	...
	public function onGroup(){
		M("mod_f2c_group")->delete("1");
		
		$gid=M("mod_f2c_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"recommend",
			"gnum"=>12,
			"status"=>1
		));
		$gid2=M("mod_f2c_group")->insert(array(
			"title"=>"推荐",
			"gkey"=>"hot",
			"gnum"=>12,
			"status"=>1
		));
		$list=M("mod_f2c_product")->selectCols(array(
			"where"=>" status=1 ",
			"limit"=>24
		));

<?php
class b2b_shop_noticeModel extends model{
	public $queue=false;
	public $table="mod_b2b_shop_notice";
	
	public function sendNewOrder($orderid){
		require_once "extends/pushtpl.class.php";
		
		$order=M("mod_b2b_order")->selectRow("orderid=".$orderid);
		if(empty($order)){
			return false;
		}
		//发送打印
		...
		if(M("module")->isInstall("printer")){
			$printer=M("mod_printer")->selectRow("tablename='b2b_shop' AND bstatus=2 AND shopid=".$order["shopid"]);
			if($printer){
				$printer_order=array(
					"money"=>$order["money"],
					"dateline"=>time(),
		...
		//发送wx模板消息
		$open=M("mod_b2b_shop_openbind")->selectRow("shopid=".SHOPID." AND k='weixin' ");
		if(empty($open)){
			return false;
		}
		...
		if($token=get_weixin_access_token()){
			$ops=array(
				"touser"=>$od["openid"],
				"template_id"=>$tpl["key"],
				"url"=>HTTP_HOST."/moduleshop.php?m=b2b_order&a=show&orderid=".$orderid,
				"data"=>array(
	...
	public function sendUpdateOrder($orderid,$type="shop"){
		...
		$url=HTTP_HOST."/module.php?m=b2b_order&a=show&orderid=".$orderid;
		if($order["status"]==1){
			$content="商家确认订单了";

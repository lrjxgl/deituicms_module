...
class car_zucheControl extends skymvc{
	...
	public function onSearch(){
		$this->smarty->display("car_zuche/search.html");
	}
	public function onList(){
		...
		$this->smarty->display("car_zuche/list.html");
	}
	...
	public function onFollow(){
		...
		$list=MM("car","car_zuche")->Dselect($ops);
		$pagelist=$this->pagelist($rscount,$limit,$url);
		$per_page=$start+$limit;
		$per_page=$per_page<$rscount?$per_page:0;
	...
	public function onShow(){
		...
		$userid=M("login")->userid;
		$data=MM("car","car_zuche")->selectRow("productid=".$productid);
		if(!$data) $this->goAll("数据出错",1);
		$shop=MM("car","car_shop")->get($data["shopid"]);
		//浏览记录
		if($userid){
			$view=M("mod_car_zuche_view")->selectRow("userid=".$userid." AND productid=".$productid);
			if(!$view){
				M("mod_car_zuche_view")->insert(array(
					"userid"=>$userid,
					"productid"=>$productid,
					"shopid"=>$data["shopid"],
					"createtime"=>date("Y-m-d H:i:s")
				));
				MM("car","car_zuche")->update(array(
		...
		//是否点赞
		$islove=0;
		$love=M("love")->selectRow("tablename='mod_car_zuche' AND userid=".$userid." AND objectid=".$productid);
		if($love){
			$islove=1;
		}
		//是否收藏
		$isfav=0;
		$fav=M("fav")->selectRow("tablename='mod_car_zuche' AND userid=".$userid." AND objectid=".$productid);
		if($fav){
			$isfav=1;
		}
		$shareLink=HTTP_HOST."/module.php?m=car_zuche&a=show&productid=".$productid."&share_userid=".$userid;
		...
		$this->smarty->goAssign(array(
			...
			"comment_tablename"=>"mod_car_zuche",
		...
		$this->smarty->display("car_zuche/show.html");
	}
	...
	public function onMy(){
		...
		$this->smarty->display("car_zuche/my.html");
	}
	public function checkPost($shopid){
		...
		$postNum=M("mod_car_zuche")->selectOne(array(
	...
	public function onSave(){
		...
		if($productid){
			...
			$productid=M("mod_car_zuche")->insert($data);
	...
	public function onCopy(){
		...
		$this->smarty->display("car_zuche/copy.html");
	}
	...
	public function onHbPay($return=0){
		...
		$car=M("mod_car_zuche")->selectRow("productid=".$productid);
		if($car["hb_on"]){

<?php
class wmo2o_shop_safephoneControl extends skymvc{
	...
	public function onDefault(){
		$data=M("mod_wmo2o_shop_safephone")->selectRow("shopid=".SHOPID);
		$this->smarty->goAssign(array(
			"data"=>$data
		));
		$this->smarty->display("wmo2o_shop_safephone/index.html");
	}
	public function onSendSms(){
		...
		$yzm=rand(1111,9999);
		$exkey="wmo2oshopsafephonesms".$telephone;
		$key="wmo2oshopsafephonesms".$telephone.$yzm;
		$t=cache()->get($exkey);
		if($t){
	...
	public function onSave(){
		...
		$row=M("mod_wmo2o_shop_safephone")->selectRow("shopid=".SHOPID);
		if($row){
			$this->goAll("安全手机不能更换，请联系客服",1);
			M("mod_wmo2o_shop_safephone")->update(array(
				"telephone"=>$telephone
			),"shopid=".SHOPID);
		}else{
			$data=array(
			...
			M("mod_wmo2o_shop_safephone")->insert($data);
		}

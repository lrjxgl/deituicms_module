<?php
	class jdo2o_cartModel extends model{
		public $table="mod_jdo2o_cart";
		...
		public function Dselect($option=array()){
			...
			if($res){
				...
				$pros=MM("jdo2o","jdo2o_product")->getListByIds($productids);
				$kss=MM("jdo2o","jdo2o_product_ks")->getListByIds($ksids);
				foreach($res as $k=> $rs){
		...
		public function getProductAmount($where){
			$list=MM("jdo2o","jdo2o_cart")->select(array("where"=>$where));
			if($list){
				foreach($list as $v){

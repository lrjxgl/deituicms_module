	...
	*/
	...
	class mmjz_productControl extends skymvc{
		...
		public function onDefault(){
			...
			$url="/moduleadmin.php?m=mmjz_product&a=default";
			$limit=20;
			...
			if($data){
				...
				$cats=MM("mmjz","mmjz_category")->getListByIds($cids);
				foreach($data as $k=>$v){
			...
			$groupList=M("mod_mmjz_group")->select(array(
				"where"=>" status=1"
			));
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=M("mod_mmjz_product")->selectRow(array("where"=>"id=".$id));
		...
		public function onSave(){
			$id=get_post("id","i");
			$data=M("mod_mmjz_product")->postData();
		...
		public function onGroup(){
			...
			$hasids=M("mod_mmjz_group_product")->selectCols(array(
			...
			if(!empty($newids)){
				foreach($newids as $productid){
					M("mod_mmjz_group_product")->insert(array(
						"gid"=>$gid,
						"productid"=>$productid,
						"orderindex"=>11
					));

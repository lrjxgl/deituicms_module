<?php
class flk_order_codeControl extends skymvc{
	
	public function __construct(){
		parent::__construct();
	}
	public function onDefault(){
		$this->smarty->display("flk_order_code/index.html");
	}
	
	public function onCheck(){
		$_GET["ajax"]=1;
		$ordercode=get("ordercode","h");
		$row=M("mod_flk_order_code")->selectRow("ordercode='".$ordercode."'");
		if(empty($row)){
			$this->goAll("验证码错误",1);
		}
	...
	public function onFinish(){
		...
		$order=MM("flk","flk_order")->selectRow("orderid=".$row["orderid"]);
		...
		M("mod_flk_order_code")->update(array(
			"isuse"=>1			
		),"orderid=".$row["orderid"]);
		$res=MM("flk","flk_order")->finish($row["orderid"]);
